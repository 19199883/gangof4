...
struct st_config_t
{
	...
	struct symbol_t	ticker_detail[2000]; /* fixed length array and specific contract number
...
{
	char st_name[64];
	struct position_t	yest_pos;
	struct position_t	today_pos;
	int	today_acc_count;
	struct acc_volume_t	today_acc_volume[10000];
};
...
{
	struct cStruct_timeval	exchg_timestamp;
	struct cStruct_timeval	local_timestamp;
	unsigned long	sig_id;
	char ticker[64];
...
{
	int count;
	struct order_request_t pending_vol[10000];
};
