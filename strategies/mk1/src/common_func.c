...
int giNightMarket = 1;  //1: night;   0: day;
int giRunModeFlg = 1;  //1: real time runing; 0: simulation;
int giStrateType = iSTRATEGY_MK1;
int giMK1Type = iMK101;


char gcExch, gcSecName[8], gcTicker[8];

int  giNoTradeItemEachRun, giLb, giLen;
int  giPosLb, giPosLen, giPendVLb, giPendVLen;

struct in_data  gstCurrIn;
struct in_data  gstPreIn;
struct in_data  gstRecCurrIn;
struct in_data gstRecPreIn;
...
struct all_parameter    gstGlobalPara;
int  giTickNo, giSumExchangeVol;
int  *gaiFullOrderBookBuy, *gaiFullOrderBookSel;
int  giFullOrderBookL, giFullOrderBookU, giLenFullOrderBook;
int  giSimuPara1, giSimuPara2, giSimuPara3;
int  giSizePerTrade;
FILE  *pfDayLogFile;
char  gcFileName[512];
int  giInitFlg;
int  giWriteOutputFileFlg, giWriteOutputFileInitFlg;
int  giInnerPar1Flg, giInnerPar2Flg, giInnerPar3Flg;
...
st_loc_trade_info gstLocTradeInfo;


int s_read_depth_market_struct(struct CDepthMarketDataField *p_stQuoteIn, struct in_data *p_stInData)
{
	char  lcTemp[20];

	p_stInData->dP = p_stQuoteIn->LastPrice;
	p_stInData->dYestP = p_stQuoteIn->PreSettlementPrice;
	p_stInData->dPYestCl = p_stQuoteIn->PreClosePrice;
	p_stInData->dYestOI = p_stQuoteIn->PreOpenInterest;
	p_stInData->dPOpen = p_stQuoteIn->OpenPrice;
	p_stInData->dPHi = p_stQuoteIn->HighestPrice;
	p_stInData->dPLo = p_stQuoteIn->LowestPrice;
	p_stInData->dTotV = p_stQuoteIn->Volume;
	p_stInData->dTotVal = p_stQuoteIn->Turnover;
	p_stInData->dOI = p_stQuoteIn->OpenInterest;
	p_stInData->dPClose = p_stQuoteIn->ClosePrice;
	p_stInData->dPSettle = p_stQuoteIn->SettlementPrice;
	p_stInData->dPLmtUp = p_stQuoteIn->UpperLimitPrice;
	p_stInData->dPLmtDn = p_stQuoteIn->LowerLimitPrice;

	strncpy(lcTemp, p_stQuoteIn->UpdateTime, strlen(p_stQuoteIn->UpdateTime));
	...
	p_stInData->iTime = p_stInData->iTime * 1000 + p_stQuoteIn->UpdateMillisec;
	if (p_stInData->iTime < 30000000) p_stInData->iTime = p_stInData->iTime + 240000000;

	strncpy(p_stInData->cTicker, p_stQuoteIn->InstrumentID, 7);

	p_stInData->dBP1 = p_stQuoteIn->BidPrice1; p_stInData->iBV1 = p_stQuoteIn->BidVolume1;
	p_stInData->dSP1 = p_stQuoteIn->AskPrice1; p_stInData->iSV1 = p_stQuoteIn->AskVolume1;
	p_stInData->dBP2 = p_stQuoteIn->BidPrice2; p_stInData->iBV2 = p_stQuoteIn->BidVolume2;
	p_stInData->dSP2 = p_stQuoteIn->AskPrice2; p_stInData->iSV2 = p_stQuoteIn->AskVolume2;
	p_stInData->dBP3 = p_stQuoteIn->BidPrice3; p_stInData->iBV3 = p_stQuoteIn->BidVolume3;
	p_stInData->dSP3 = p_stQuoteIn->AskPrice3; p_stInData->iSV3 = p_stQuoteIn->AskVolume3;
	p_stInData->dBP4 = p_stQuoteIn->BidPrice4; p_stInData->iBV4 = p_stQuoteIn->BidVolume4;
	p_stInData->dSP4 = p_stQuoteIn->AskPrice4; p_stInData->iSV4 = p_stQuoteIn->AskVolume4;
	p_stInData->dBP5 = p_stQuoteIn->BidPrice5; p_stInData->iBV5 = p_stQuoteIn->BidVolume5;
	p_stInData->dSP5 = p_stQuoteIn->AskPrice5; p_stInData->iSV5 = p_stQuoteIn->AskVolume5;
	p_stInData->cExchCode = gcExch;
...
int s_set_exch_parameters(char lcExch, char *lcSec, struct all_parameter *p_stOutData)
{
	...
	if( lcExch == cSHFE)
    {
		if (giNightMarket == 0)
		{
			...
			p_stOutData->stSec.iSecTradeStartTime = p_stOutData->stExch.iMktStartTime + 1000;
            p_stOutData->stSec.iSecTradeEndTime = 112500;
            p_stOutData->stSec.iSecTradeStartTime02 = 134000;
            p_stOutData->stSec.iSecTradeEndTime02 = 144000;
		...
		else if (giNightMarket == 1)
		{
			...
			p_stOutData->stSec.iSecTradeStartTime = p_stOutData->stExch.iMktStartTime + 1000;
			p_stOutData->stSec.iSecTradeEndTime = 232500;
			...
			if(strcmp(gcSecName, "ru") == 0 || strcmp(gcSecName, "RU") == 0 || strcmp(gcSecName, "hc") == 0 || strcmp(gcSecName, "HC") == 0 || strcmp(gcSecName, "rb") == 0 || strcmp(gcSecName, "RB") == 0
		...
		p_stOutData->stSec.iMinTradeCount = 0;
		p_stOutData->stSec.iMinTradeSec = 0;
		p_stOutData->stSec.iMaxSizePerOrder = 500;
		p_stOutData->stSec.iUseLastV = iNO;
		p_stOutData->stSec.rTOverPct = 0.1;
		p_stOutData->stSec.iRatioLevel = 10;
		p_stOutData->stSec.iLookBackRecordNo = 0;
	...
	else if( lcExch == cDCE)
    {
		...
		else if (giNightMarket == 1)
		{
			...
			p_stOutData->stSec.iSecTradeStartTime = p_stOutData->stExch.iMktStartTime + 1000;
			p_stOutData->stSec.iSecTradeEndTime = 231500;
		...
		p_stOutData->stSec.rTVCompRatio = 0.5;
		p_stOutData->stSec.iMinTradeCount = 0;
		p_stOutData->stSec.iMinTradeSec = 1;
		p_stOutData->stSec.iMaxSizePerOrder = 1000;
		p_stOutData->stSec.iUseLastV = iYES;
		p_stOutData->stSec.rTOverPct = 0.5;
		p_stOutData->stSec.iRatioLevel = 10;
		p_stOutData->stSec.iLookBackRecordNo = 0;
		p_stOutData->stSec.iBaseSize = 0;
    ...
    else if( lcExch == cCZCE)
	{
		...
		p_stOutData->stSec.iMaxSizePerOrder = 1000;
		p_stOutData->stSec.iUseLastV = iNO;
		p_stOutData->stSec.rTOverPct = 0.50;
		p_stOutData->stSec.iRatioLevel = 10;
		p_stOutData->stSec.iLookBackRecordNo = 0;
		p_stOutData->stSec.iBaseSize = 0;
	...
	else if(lcExch == cCFFE)
    {
		...
		p_stOutData->stSec.iSecTradeStartTime = p_stOutData->stExch.iMktStartTime + 1000;
        p_stOutData->stSec.iSecTradeEndTime = 145500;
        p_stOutData->stSec.iSecTradeStartTime02 = -1;
        p_stOutData->stSec.iSecTradeEndTime02 = -1;
		...
		p_stOutData->stSec.rTOverPct = 0.1;
		p_stOutData->stSec.iRatioLevel = 10;
		p_stOutData->stSec.iLookBackRecordNo = 120;
		p_stOutData->stSec.iBaseSize = 0;
...
int s_set_convertion_parameters(char *lcSecName, struct sec_parameter *p_stOutData)
{
	...
	else if(strncmp(cSeclow, "ag", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.0000525;
	...
	else if(strncmp(cSeclow, "au", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.00525; p_stOutData->rRssTCostByVal = 0.0;
	...
	{
		p_stOutData->rTCostByVol = 0.01 ; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.3150; p_stOutData->rRssTCostByVal = 0.0;
	...
	{
		p_stOutData->rTCostByVol = 0.0 ; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.000105;
	...
	else if(strncmp(cSeclow, "cu", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00002074;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000045 ;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00008295;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.00000 ;
		p_stOutData->rRssTCostByVol = 4.977; p_stOutData->rRssTCostByVal = 0.0;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.0000450;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00004725;
		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 2;
	...
	else if(strncmp(cSeclow, "zn", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.3150; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0;
	}
	else if(strncmp(cSeclow, "a", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.205; p_stOutData->rRssTCostByVal = 0.0    ;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.00010; p_stOutData->rExchSameDayDisc = 0.50;  gstGlobalPara.stSec.rBrokerPct = 0.050;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.0000525 ;
	...
	else if(strncmp(cSeclow, "cs", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.15375; p_stOutData->rRssTCostByVal = 0.0  ;
	...
	else if(strncmp(cSeclow, "v", 2) == 0)
	{
		p_stOutData->rTCostByVol = 0.120; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.205; p_stOutData->rRssTCostByVal = 0.0  ;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2; p_stOutData->iMaxSizePerOrder = 2000;
		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
	}
	else if(strncmp(cSeclow, "c", 2) == 0)
	{
		p_stOutData->rTCostByVol = 0.120; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.0615; p_stOutData->rRssTCostByVal = 0.0  ;
	...
	else if(strncmp(cSeclow, "i", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00003075 ;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000080; p_stOutData->rExchSameDayDisc = 0.50;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.0000738;
	...
	else if(strncmp(cSeclow, "jd", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.0001538;
	...
	{
		p_stOutData->rTCostByVol = 0.50; p_stOutData->rTCostByVal = 0.0; p_stOutData->rExchSameDayDisc = 0.50; gstGlobalPara.stSec.rBrokerPct = 0.050;
		p_stOutData->rRssTCostByVol = 0.41; p_stOutData->rRssTCostByVal = 0.0  ;
	...
	{
		p_stOutData->rTCostByVol = 0.20; p_stOutData->rTCostByVal = 0.0; p_stOutData->rExchSameDayDisc = 0.50; ; gstGlobalPara.stSec.rBrokerPct = 0.050;
		p_stOutData->rRssTCostByVol = 0.15375; p_stOutData->rRssTCostByVal = 0.0 ;
		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
	...
	else if(strncmp(cSeclow, "p", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.2563; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
	...
	{
		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000050; p_stOutData->rExchSameDayDisc = 0.50; gstGlobalPara.stSec.rBrokerPct = 0.050;
		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.000246;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
	...
	{
		p_stOutData->rTCostByVol = 0.150; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.3150; p_stOutData->rRssTCostByVal = 0.00;
	...
	else if(strncmp(cSeclow, "cf", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 1.26; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0;
		p_stOutData->iRatioLevel = 10;
	}
	else if(strncmp(cSeclow, "sr", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.1650; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
		p_stOutData->iRatioLevel = 10;
	}
	else if(strncmp(cSeclow, "ta", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.63; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
		p_stOutData->rTickSize = 2.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 2; p_stOutData->iDecimalPt = 0;
		p_stOutData->iRatioLevel = 10;
	}
	...
	{
		p_stOutData->rTCostByVol = 0.3; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.147; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
		p_stOutData->rTickSize = 2.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
		p_stOutData->iRatioLevel = 10;
	}
	else if(strncmp(cSeclow, "zc", 2) == 0)
	{
		...
		p_stOutData->rRssTCostByVol = 0.01232; p_stOutData->rRssTCostByVal = 0.00;
		p_stOutData->iUnitPerContract = 100; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
		p_stOutData->rTickSize = 0.2; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
		p_stOutData->iRatioLevel = 10;
	}
	...
	{
		p_stOutData->rTCostByVol = 3.0 / 20.0; p_stOutData->rTCostByVal = 0.0;
		p_stOutData->rRssTCostByVol = 0.0825; p_stOutData->rRssTCostByVal = 0.00;
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.00019; p_stOutData->rRssTCostByVal = 0.0;
//		p_stOutData->iUnitPerContract = 10000; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 0.005; p_stOutData->iPMultiplier = 10; p_stOutData->iTickSizeMulti = 2; p_stOutData->iDecimalPt = 1;
//		p_stOutData->iRatioLevel = 15;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000050;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00003315;
//		p_stOutData->iUnitPerContract = 15; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 20;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.01 ; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.003315; p_stOutData->rRssTCostByVal = 0.0;
//		p_stOutData->iUnitPerContract = 1000; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 0.050; p_stOutData->iPMultiplier = 100; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 2;
//		p_stOutData->iRatioLevel = 35;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.01 ; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.1989; p_stOutData->rRssTCostByVal = 0.0;
//		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 100; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 2;
//		p_stOutData->iRatioLevel = 35;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0 ; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00001989;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 2.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 35;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000025 ;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.000016575;
//		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 10.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 10; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 30;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.0000450;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00001492;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 30;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.0000450;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00002984;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0;
//	}
//	else if(strncmp(cSeclow, "zn", 2) == 0)
//	{
//		p_stOutData->rTCostByVol = 3.0/5; p_stOutData->rTCostByVal = 0.000000;
//		p_stOutData->rRssTCostByVol = 0.1989; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0;
//	}
//	else if(strncmp(cSeclow, "ni", 2) == 0)
...
//		p_stOutData->rRssTCostByVol = 3.978; p_stOutData->rRssTCostByVal = 0.0  ;
//		p_stOutData->iUnitPerContract = 1; p_stOutData->iWrongVol = 50000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 10.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0 ;
//	}
//	else if(strncmp(cSeclow, "a", 2) == 0)
//	{
//		p_stOutData->rTCostByVol = 0.20; p_stOutData->rTCostByVal = 0.0; p_stOutData->rExchSameDayDisc = 0.00;
//		p_stOutData->rRssTCostByVol = 0.110; p_stOutData->rRssTCostByVal = 0.0    ;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 50;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.00010; p_stOutData->rExchSameDayDisc = 0.50;  gstGlobalPara.stSec.rBrokerPct = 0.050;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.000055 ;
//		p_stOutData->iUnitPerContract = 500; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 0.050; p_stOutData->iPMultiplier = 100; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 2;
//		p_stOutData->iRatioLevel = 10; p_stOutData->iWithCancel = iYES;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.120; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.1575; p_stOutData->rRssTCostByVal = 0.0  ;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2; p_stOutData->iMaxSizePerOrder = 2000;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//	}
//	else if(strncmp(cSeclow, "c", 2) == 0)
//	{
//		p_stOutData->rTCostByVol = 0.120; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.20660; p_stOutData->rRssTCostByVal = 0.0  ;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1; p_stOutData->iMaxSizePerOrder = 2000;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//	}
//	else if(strncmp(cSeclow, "fb", 2) == 0)
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000080; p_stOutData->rExchSameDayDisc = 0.50;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.000033 ;
//		p_stOutData->iUnitPerContract = 100; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 0.50; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 1;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000080; p_stOutData->rExchSameDayDisc = 0.50;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.0000330;
//		p_stOutData->iUnitPerContract = 100; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2; p_stOutData->iMaxSizePerOrder = 500;
//		p_stOutData->rTickSize = 0.50; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 1;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000150; p_stOutData->rExchSameDayDisc = 0.00;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00015750;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2; p_stOutData->iMaxSizePerOrder = 300;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 20; p_stOutData->iWithCancel = iYES;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.20; p_stOutData->rTCostByVal = 0.0; p_stOutData->rExchSameDayDisc = 0.50; ; gstGlobalPara.stSec.rBrokerPct = 0.050;
//		p_stOutData->rRssTCostByVol = 0.08250; p_stOutData->rRssTCostByVal = 0.0 ;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 15; p_stOutData->iWithCancel = iYES;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.250; p_stOutData->rTCostByVal = 0.0; ; p_stOutData->rExchSameDayDisc = 0.00;
//		p_stOutData->rRssTCostByVol = 0.13750; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 2.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 2; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.0; p_stOutData->rTCostByVal = 0.000050; p_stOutData->rExchSameDayDisc = 0.50; gstGlobalPara.stSec.rBrokerPct = 0.050;
//		p_stOutData->rRssTCostByVol = 0.0; p_stOutData->rRssTCostByVal = 0.00002750;
//		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 45; p_stOutData->iWithCancel = iYES;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.150; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.1575; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 1.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 4.30 / 5.0; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.4730; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 5; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 2;
//		p_stOutData->rTickSize = 5.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 5; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 0.3; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.263; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 10; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 2.0; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 10;
//	}
...
//	{
//		p_stOutData->rTCostByVol = 3.0/20.0; p_stOutData->rTCostByVal = 0.0;
//		p_stOutData->rRssTCostByVol = 0.022; p_stOutData->rRssTCostByVal = 0.00;
//		p_stOutData->iUnitPerContract = 100; p_stOutData->iWrongVol = 100000; p_stOutData->iCostSide = 1;
//		p_stOutData->rTickSize = 0.2; p_stOutData->iPMultiplier = 1; p_stOutData->iTickSizeMulti = 1; p_stOutData->iDecimalPt = 0;
//		p_stOutData->iRatioLevel = 10;
//	}
...
int s_read_zz_fut_struct(struct czce_my_deep_quote *p_stcZZQuoteIn, struct in_data *p_stInData)
{
	...
	p_stInData->dWeightedBuyOrderP = 0.0;
	p_stInData->dWeightedSelOrderP = 0.0 ;
...
//----------------------------------------------------
double f_calc_curr_avgp(double lrVal, int liV, double lrPrevAvgP)
{
	double dres;
	if (liV > 0 && lrVal > 0.0)
		dres = lrVal / liV;
	else
		dres = lrPrevAvgP * gstGlobalPara.stSec.iUnitPerContract;
...
int f_find_security_name2(char *lcTicker, char* sc_name, char* pExch)
{
    int i;
    char lcExch;

    strncpy(sc_name, lcTicker, strlen(lcTicker)+1);
    for(i=0; i<strlen(lcTicker); i++)
    {
        if(sc_name[i] >= '0' && sc_name[i]<= '9') {sc_name[i] = '\0'; break;}
    }

    lcExch = 'A';
    if(strcmp(sc_name,"m")==0 || strcmp(sc_name,"y")==0 || strcmp(sc_name,"a")==0 ||
       strcmp(sc_name,"b")==0 || strcmp(sc_name,"p")==0 || strcmp(sc_name,"c")==0 ||
       strcmp(sc_name,"cs")==0 || strcmp(sc_name,"jd")==0 || strcmp(sc_name,"bb")==0 ||
       strcmp(sc_name,"fb")==0 || strcmp(sc_name,"l")==0 || strcmp(sc_name,"v")==0 ||
       strcmp(sc_name,"pp")==0 || strcmp(sc_name,"j")==0 || strcmp(sc_name,"jm")==0 ||
       strcmp(sc_name,"i")==0)
    {
        lcExch = 'B';
    }
    else if(strcmp(sc_name,"SR")==0 || strcmp(sc_name,"CF")==0 || strcmp(sc_name,"ZC")==0 ||
       strcmp(sc_name,"FG")==0 || strcmp(sc_name,"TA")==0 || strcmp(sc_name,"MA")==0 ||
       strcmp(sc_name,"WH")==0 || strcmp(sc_name,"PM")==0 || strcmp(sc_name,"RI")==0 ||
       strcmp(sc_name,"LR")==0 || strcmp(sc_name,"JR")==0 || strcmp(sc_name,"RS")==0 ||
       strcmp(sc_name,"OI")==0 || strcmp(sc_name,"RM")==0 || strcmp(sc_name,"SF")==0 ||
       strcmp(sc_name,"SM")==0)
    {
        lcExch = 'C';
    }
