         ...
         struct all_parameter *lstGlobalPara, st_loc_trade_info *p_stLocTradeInfo)
{
	...
	icnt_long[0] = 0; icnt_long[1] = 0;
	icnt_short[0] = 0; icnt_short[1] = 0;
	...
	for(i=0; i<giMaxMyOrder; i++)
    {
        if(gstOrderBuy[i].sig_id != 0)
        {
            ...
            {
                icnt_long[0] = icnt_long[0] + gstOrderBuy[i].volume - gstOrderBuy[i].exec_acc_vol;
            }
        }
        if(gstOrderSel[i].sig_id != 0)
        {
            ...
            {
                icnt_short[0] = icnt_short[0] + gstOrderSel[i].volume - gstOrderSel[i].exec_acc_vol;
            }
        }
    }

    iVol_target_long[0] = p_stLocTradeInfo->iPosK - icnt_long[0];
    iVol_target_short[0] = p_stLocTradeInfo->iPosD - icnt_short[0];
        ...
        else if(gstUsrOrderIn[i1].iBS == USER_ORDER_CANCEL) //cancel order
        {
            for(i=0; i<giMaxMyOrder; i++)
            {
                if(gstOrderBuy[i].sig_id != 0 && fabs(gstOrderBuy[i].price - gstUsrOrderIn[i1].dPrice)< 0.001)
                {
                    ...
                    {
                        s_set_mm_cancel_order(gstOrderBuy[i].sig_id);
                ...
                if(gstOrderSel[i].sig_id != 0 && fabs(gstOrderSel[i].price - gstUsrOrderIn[i1].dPrice)< 0.001)
                {
                    ...
                    {
                        s_set_mm_cancel_order(gstOrderSel[i].sig_id);
...
//    if(gstOrderSel[i].sig_id == 33 || gstOrderSel[i].sig_id == 32)
//    {
//        printf("sig_id =%d, gstOrderSel[i].itime=%d, lstCurrInData->iTime=%d\n",gstOrderSel[i].sig_id, gstOrderSel[i].itime,
//               lstCurrInData->iTime);
//    }
//    if(gstOrderBuy[i].sig_id == 33 || gstOrderBuy[i].sig_id == 32)
//    {
//        printf("sig_id =%d, gstOrderBuy[i].itime=%d, lstCurrInData->iTime=%d\n",gstOrderBuy[i].sig_id, gstOrderBuy[i].itime,
//               lstCurrInData->iTime);
//    }
//    if(gstOrderCancel[i].sig_id == 33 || gstOrderCancel[i].sig_id == 32)
//    {
//        printf("sig_id =%d, gstOrderCancel[i].itime=%d, lstCurrInData->iTime=%d\n",gstOrderBuy[i].sig_id, gstOrderCancel[i].itime,
//               lstCurrInData->iTime);
//    }

#ifdef WRITE_SYSLOG_

    syslog(LOG_ERR, "itime=%d, %8.1f, %8.1f\n",lstCurrInData->iTime, lstCurrInData->dBP[0], lstCurrInData->dSP[0]);
    syslog(LOG_ERR, "sell order:\n");
    for(i=0; i<giMaxMyOrder; i++)
    {
        if(gstOrderSel[i].sig_id != 0 && gstOrderSel[i].status == SIG_STATUS_INIT)
            syslog(LOG_ERR, "my Sell %d %8.1f %d %d\n",gstOrderSel[i].sig_id, gstOrderSel[i].price, gstOrderSel[i].volume, gstOrderSel[i].vol2Pass);
    }
    syslog(LOG_ERR, "buy order:\n");
    for(i=0; i<giMaxMyOrder; i++)
    {
        if(gstOrderBuy[i].sig_id != 0 && gstOrderBuy[i].status == SIG_STATUS_INIT)
            syslog(LOG_ERR, "my Buy %d %8.1f %d %d\n",gstOrderBuy[i].sig_id, gstOrderBuy[i].price, gstOrderBuy[i].volume, gstOrderBuy[i].vol2Pass);
    }

    syslog(LOG_ERR, "cancel order:\n");
    for(i=0; i<giMaxMyOrder*2; i++)
    {
        if(gstOrderCancel[i].sig_id != 0 && gstOrderCancel[i].status == SIG_STATUS_INIT)
            syslog(LOG_ERR, "my cancel %d %d\n",gstOrderCancel[i].sig_id, gstOrderCancel[i].cancel_sig_id);
    }
#endif // WRITE_SYSLOG_
...
//                if(gstOrderSel[i].itime == 0)
//                    {
//                        printf("itime = 0, sig_id = %d gstCurrSigId=%d\n", gstOrderSel[i].sig_id, gstCurrSigId);
//                    }
...
//                    }
    ...
    {
        if(gstUsrOrderOut[i].dPrice < gstUsrOrderOut[i2].dPrice)
        {
...
int s_set_mm_cancel_order(const int sig_id)
{
    int i;
...
{
#ifdef WRITE_SYSLOG_
    syslog(LOG_ERR, "call s_mm_assign_out_tsignal at %d, tick = %d\n", gstCurrIn.iTime,giTickNo);
#endif // WRITE_SYSLOG_
    int i;
    int numSignal;
    int liNumOrderlist;
    ...
    *licNoTSignal = 0;
    ...
    for(i=0; i<liNumOrderlist*2; i++)
    {
        if(gstOrderCancel[i].status == SIG_STATUS_INIT && gstOrderCancel[i].direct == SIG_DIRECTION_CANCEL)
        {
            ...
            strncpy(pstTSignal[numSignal].ticker, gstCurrIn.cTicker, 7);
    ...
    *licNoTSignal = numSignal;
    giNoTradeItemEachRun = numSignal;
...
int s_update_my_order_info(struct signal_resp_t *lstStructOrderStatusIn)
{
	...
	if(strcmp(lstStructOrderStatusIn->ticker, gcTicker) != 0) return 1;
	...
	if(lstStructOrderStatusIn->sig_act == 1) //buy
	{
        ...
        {
            gstLocTradeInfo.iTotalCancelCount ++;
            for(i=0; i< liNumOrderlist*2; i++)
            {
                if(gstOrderCancel[i].cancel_sig_id == lstStructOrderStatusIn->sig_id)
                {
#ifdef WRITE_SYSLOG_
    syslog(LOG_ERR, "do order_canceled id=%d\n",gstOrderCancel[i].cancel_sig_id);
#endif // WRITE_SYSLOG_
            ...
            for(i=0; i< liNumOrderlist; i++)
            {
                if(gstOrderBuy[i].sig_id == lstStructOrderStatusIn->sig_id)
                {
                    ...
                    if(gstOrderBuy[i].exec_vol != 0)
                    {
                        ...
                        s_mm_calc_profitloss_and_update(&(gstOrderBuy[i]), &gstGlobalPara.stSec, &gstLocTradeInfo);
                        gstLocTradeInfo.iLastTradeTime = gstCurrIn.iTime;
                    }
                    break;
                }
            }
            ...
            for(i=0; i< liNumOrderlist; i++)
            {
                if(gstOrderSel[i].sig_id == lstStructOrderStatusIn->sig_id)
                {
                    ...
                    if(gstOrderSel[i].exec_vol != 0)
                    {
                        ...
                        s_mm_calc_profitloss_and_update(&(gstOrderSel[i]), &gstGlobalPara.stSec, &gstLocTradeInfo);
                        gstLocTradeInfo.iLastTradeTime = gstCurrIn.iTime;
                    }
                    break;
                }
	...
	int *laiFullOrderBookBuy, int *laiFullOrderBookSel, int liLenFullOrderBook)
{
	...
	int  liVLo, liVHi, liVol2, liPosNew, liIndex, liLevel, liFrom, liTo, liI, i, liPreVol;
    ...
    for(i=0; i< giMaxMyOrder; i++)
    {
		...
		if(gstOrderBuy[i].price > lrPriceH - lrTick/2)
		{
			...
			else
			{
				if(gstOrderBuy[i].exec_acc_vol==0)
                    liVol2 = (int)((p_stDataIn->iVol - gstOrderBuy[i].vol2Pass)*lrExeRatio);
                else
		...
		if(gstOrderBuy[i].exec_vol != 0 )
        {
            if(gstOrderBuy[i].kaiping == SIG_ACTION_OPEN)
                gstLocTradeInfo.iPosD += gstOrderBuy[i].exec_vol;
            else if(gstOrderBuy[i].kaiping == SIG_ACTION_FLAT)
                gstLocTradeInfo.iPosK -= gstOrderBuy[i].exec_vol;
	...
	for(i=0; i< giMaxMyOrder; i++)
	{
		...
		if(gstOrderSel[i].price < lrPriceL + lrTick/2)
		{
			...
			else
			{
				if(gstOrderSel[i].exec_acc_vol==0)
                    liVol2 = (int)((p_stDataIn->iVol - gstOrderSel[i].vol2Pass)*lrExeRatio);
                else
		...
		if(gstOrderSel[i].exec_vol != 0 )
        {
            if(gstOrderSel[i].kaiping == SIG_ACTION_OPEN)
                gstLocTradeInfo.iPosK += gstOrderSel[i].exec_vol;
            else if(gstOrderSel[i].kaiping == SIG_ACTION_FLAT)
                gstLocTradeInfo.iPosD -= gstOrderSel[i].exec_vol;
	...
	int *laiFullOrderBookBuy, int *laiFullOrderBookSel, int liLenFullOrderBook)
{
	...
	int  liVol2, liPosNew, liIndex, liLevel, liFrom, liTo, liI, i, liPreVol;
	double larBP[30], larSP[30];
	int laiBV[30], laiSV[30];
	int isum1,isum2;
	double dsum1, dsum2;
