...
XSpeedTradeContext::XSpeedTradeContext()
{
    cur_req_id_ = 0;
...
void XSpeedTradeContext::InitOrderRef(OrderRefDataType orderref_prefix)
{
    cur_max_order_ref_ = orderref_prefix * 10000000;
}

// 设置报单引用
void XSpeedTradeContext::SetOrderRef(OrderRefDataType cur_max_order_ref)
{
...
OrderRefDataType XSpeedTradeContext::GetNewOrderRef()
{
    lock_guard<mutex> lock(sync_mutex_);
    ++cur_max_order_ref_;
...
OrderRefDataType XSpeedTradeContext::GetNewOrderRefForInsertQuote()
{
    lock_guard<mutex> lock(sync_mutex_);
...
int XSpeedTradeContext::GetRequestID()
{
    lock_guard<mutex> lock(sync_mutex_);
    return ++cur_req_id_;
}

void XSpeedTradeContext::SaveOrderInfo(OrderRefDataType order_id, const XSpeedOrderInfo& order_info)
{
    long serial_no = order_info.serial_no;
    lock_guard<mutex> lock(order_mutex_);
...
const XSpeedOrderInfo* XSpeedTradeContext::GetOrderInfoByLocalID(OrderRefDataType local_id)
{
    lock_guard<mutex> lock(order_mutex_);
...
void XSpeedTradeContext::UpdateCancelInfoOfOrderRef(OrderRefDataType order_ref, long c_s_no)
{
    lock_guard<mutex> lock(order_mutex_);
...
OrderRefDataType XSpeedTradeContext::GetOrderRefBySerialNo(long serial_no)
{
...
void XSpeedTradeContext::SaveForquoteSerialNoOfOrderRef(OrderRefDataType order_ref, long sn)
{
...
long XSpeedTradeContext::GetForquoteSerialNoOfOrderRef(OrderRefDataType order_ref)
{
    lock_guard<mutex> lock(forquote_orderref_to_serialno_mutex_);
    OrderRefToSerialNoMap::iterator it = forquote_orderref_to_serialno_.find(order_ref);
    if (it == forquote_orderref_to_serialno_.end())
...
void XSpeedTradeContext::SaveQuoteInfo(OrderRefDataType quote_ref, const XSpeedQuoteInfo &quote_info)
{
...
const XSpeedQuoteInfo* XSpeedTradeContext::GetQuoteInfoByOrderRef(OrderRefDataType order_ref)
{
    lock_guard<mutex> lock(quote_mutex_);
...
void XSpeedTradeContext::UpdateCancelInfoOfQuoteRef(OrderRefDataType order_ref, long c_s_no)
{
    lock_guard<mutex> lock(quote_mutex_);
...
OrderRefDataType XSpeedTradeContext::GetQuoteRefBySerialNo(long serial_no)
{
    OrderRefDataType quote_ref = 0;
    lock_guard<mutex> lock(quote_mutex_);
