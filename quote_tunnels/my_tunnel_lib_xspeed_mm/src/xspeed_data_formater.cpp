...
std::string XSpeedDatatypeFormater::ToString(const DFITCCancelAllOrderField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCCancelAllOrderField\n"
            "    lRequestID=%ld\n"
            "    accountID=%s\n"
            "    exchangeID=%s\n",
            p->lRequestID,                   //请求ID
            p->accountID,                    //资金账户ID
            p->exchangeID                    //交易所编码(目前只支持大商所)
            );
    }
    else
    {
        snprintf(buf, sizeof(buf), "structName=DFITCCancelAllOrderField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCForQuoteField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteField\n"
            "    lRequestID=%ld\n"
            "    accountID=%s\n"
            "    instrumentID=%s\n",
            p->lRequestID,                   //请求ID
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteField <null>");
    }
...
std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteSubscribeRspField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteSubscribeRspField\n"
            "    lRequestID=%ld\n"
            "    subscribeFlag=%d\n"
            "    exchangeID=%s\n",
            p->lRequestID,                   //请求ID
            p->subscribeFlag,                //订阅状态
            p->exchangeID                    //交易所
            );
    }
    else
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteSubscribeRspField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteSubscribeRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteSubscribeRtnField\n"
            "    quoteID=%s\n"
            "    exchangeID=%s\n"
            "    instrumentID=%s\n"
            "    source=%d\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteSubscribeRtnField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteUnSubscribeRspField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteUnSubscribeRspField\n"
            "    lRequestID=%ld\n"
            "    subscribeFlag=%d\n"
            "    exchangeID=%s\n",
            p->lRequestID,                  //请求ID
            p->subscribeFlag,               //退订状态
            p->exchangeID                   //交易所
            );
    }
    else
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteUnSubscribeRspField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteRspField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteRspField\n"
            "    localOrderID=%ld\n"
            "    spdOrderID=%ld\n"
            "    lRequestID=%ld\n"
            "    fee=%.4f\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteRspField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteRtnField\n"
            "    exchangeID=%s\n"
            "    clientID=%s\n"
            "    orderSysID=%s\n"
            "    instrumentID=%s\n"
...
//            "    closeID=%s\n"
            ...
            p->sOpenCloseType,                //开平标志（卖）
            p->speculator,                    //投资类别
            p->bOrderAmount,                  //委托数量（买）
            p->sOrderAmount,                  //委托数量（卖）
            p->bInsertPrice,                  //委托价  （买）
            p->sInsertPrice,                  //委托价  （卖）
            p->spdOrderID,                    //柜台委托号
            p->accountID,                     //资金账号
            p->instrumentType,                //合约类型
            p->suspendTime,                   //挂单时间
//            p->closeID,                       //平仓执行单号
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteRtnField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteMatchRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteMatchRtnField\n"
            "    exchangeID=%s\n"
            "    clientID=%s\n"
            "    instrumentID=%s\n"
            "    seatCode=%s\n"
            ...
            p->margin,                       //开仓为保证金,平仓为解冻保证金
            p->buySellType,                  //买卖
            p->closeTodayAmount,             //平今数量
            p->closePrice,                   //平仓金额
            p->closeTodayPrice,              //平今金额
            p->adjustmentInfo,               //组合或对锁的保证金调整信息,格式:[合约代码,买卖标志,投资类别,调整金额,]
            p->frozenCapita,                 //成交解冻委托冻结的资金
            p->dateCloseProfitLoss,          //盯市平仓盈亏
            p->instrumentType,               //合约类型
            p->sessionID,                    //会话标识
            p->largeMarginDirect,            //大边保证金方向
            p->quoteID                       //询价编号
            );
    }
    else
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteMatchRtnField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteCanceledRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteCanceledRtnField\n"
            "    exchangeID=%s\n"
            "    clientID=%s\n"
            "    orderSysID=%s\n"
            "    instrumentID=%s\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteCanceledRtnField <null>");
    }
...
std::string XSpeedDatatypeFormater::ToString(const DFITCCancelAllOrderRspField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCCancelAllOrderRspField\n"
            "    lRequestID=%ld\n"
            "    accountID=%s\n"
            "    orderStatus=%d\n",
            p->lRequestID,                   //请求ID
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCCancelAllOrderRspField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCForQuoteRspField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteRspField\n"
            "    lRequestID=%ld\n"
            "    spdOrderID=%ld\n"
            "    commTime=%s\n",
            p->lRequestID,                   //请求ID
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteRspField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCForQuoteRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteRtnField\n"
            "    spdOrderID=%ld\n"
            "    sessionID=%ld\n"
            "    instrumentID=%s\n"
            "    exchangeID=%s\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCForQuoteRtnField <null>");
    }
...
std::string XSpeedDatatypeFormater::ToString(const DFITCQuoteOrderRtnField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteOrderRtnField\n"
            "\tlRequestID=%ld\n"
            "\tspdOrderID=%ld\n"
            "\torderStatus=%d\n"
            "\tinstrumentID=%s\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCQuoteOrderRtnField <null>");
    }

    return buf;
}

std::string XSpeedDatatypeFormater::ToString(const DFITCSpecificInstrumentField* p)
{
    char buf[2048];

    if (p)
    {
        snprintf(buf, sizeof(buf), "structName=DFITCSpecificInstrumentField\n"
            "    lRequestID=%ld\n"
            "    accountID=%s\n"
            "    InstrumentID=%s\n"
            "    exchangeID=%s\n"
    ...
    {
        snprintf(buf, sizeof(buf), "structName=DFITCSpecificInstrumentField <null>");
    }
