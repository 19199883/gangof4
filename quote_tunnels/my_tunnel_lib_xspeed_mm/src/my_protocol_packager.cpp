...
void XSpeedPacker::QuoteTrade(char dir, const DFITCQuoteMatchRtnField* pf, const XSpeedQuoteInfo* p_req, T_QuoteTrade& rtn)
{
    memset(&rtn, 0, sizeof(rtn));

    rtn.entrust_no = p_req->entrust_no;
    rtn.serial_no = p_req->serial_no;
    memcpy(rtn.stock_code, pf->instrumentID, sizeof(rtn.stock_code));
    rtn.direction = dir;
    rtn.business_volume = pf->matchedAmount;
    rtn.business_price = pf->matchedPrice;
    rtn.business_no = atoi(pf->matchID);
