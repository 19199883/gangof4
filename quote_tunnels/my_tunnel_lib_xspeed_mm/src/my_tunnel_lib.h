...
class XspeedTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    XspeedTunnel(const std::string &xspeed_config_file);
    virtual ~XspeedTunnel();
...
private:
    // 禁止拷贝和赋值
    XspeedTunnel(const XspeedTunnel & other);
    XspeedTunnel operator=(const XspeedTunnel & other);
    ...
    static void RespondHandleThread(XspeedTunnel *ptunnel);
};
