...
class FSTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    FSTunnel(const std::string &ctp_config_file);
    virtual ~FSTunnel();
...
private:
    // 禁止拷贝和赋值
    FSTunnel(const FSTunnel & other);
    FSTunnel operator=(const FSTunnel & other);
    ...
    static void RespondHandleThread(FSTunnel *ptunnel);
};
