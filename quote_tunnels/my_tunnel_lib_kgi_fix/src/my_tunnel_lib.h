...
class KgiFixTunnel: public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    KgiFixTunnel(const std::string &ctp_config_file);
    virtual ~KgiFixTunnel();
...
private:
    // 禁止拷贝和赋值
    KgiFixTunnel(const KgiFixTunnel & other);
    KgiFixTunnel operator=(const KgiFixTunnel & other);
    ...
    static void RespondHandleThread(KgiFixTunnel *ptunnel);
};
