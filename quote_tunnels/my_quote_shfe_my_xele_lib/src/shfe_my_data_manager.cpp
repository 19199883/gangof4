...
MYShfeMDManager::MYShfeMDManager(const ConfigData &cfg)
    : cfg_(cfg)
{
    pthread_spin_init(&mbl_mutex_, 0);
    pthread_spin_init(&depth_mutex_, 0);
...
MYShfeMDManager::~MYShfeMDManager()
{
    ...
    pthread_spin_destroy(&mbl_mutex_);
    pthread_spin_destroy(&depth_mutex_);
}

void MYShfeMDManager::OnMBLData(const CShfeFtdcMBLMarketDataField* const pdata, bool last_flag)
{
    _TTSpinLockGuard guard(mbl_mutex_);
    if (pdata)
    {
...
void* MYShfeMDManager::ProcessThread(MYShfeMDManager* p_mngr)
{
    ...
    std::string cur_code;
    MBLDataCollection data_tmp;
    data_tmp.reserve(10000);

    while (p_mngr->running_flag_)
    {
        {
            // 数据交换
            _TTSpinLockGuard guard(p_mngr->mbl_mutex_);

            if (!p_mngr->data_in_.empty())
            {
                p_mngr->data_in_.swap(data_tmp);
            }
        }
        ...
        for (MBLDataCollection::iterator p = data_tmp.begin(); p != data_tmp.end(); ++p)
        {
            if (p->isLast)
            {
            ...
            if (p->field.Volume > 0)
            {
                ...
                // 数据记录
                SHFEMDQuoteSnapshot * p_cur_snapshot = p_mngr->PushDataToBuffer(cur_code, p);

                (void) p_cur_snapshot;
//                if (p_cur_snapshot && p_cur_snapshot->sell_count >= MY_SHFE_QUOTE_PRICE_POS_COUNT)
//                {
//                    // try send type 3 data to client as soon as possible!
//                }
...
// 买方向合约记录
                ...
                else if (prev_dir == SHFE_FTDC_D_Sell && cur_dir == SHFE_FTDC_D_Sell && prev_code != cur_code)
                {
                    SHFEMDQuoteSnapshot * p_snapshot_prev = p_mngr->GetDataCache(prev_code);
                    p_mngr->SendToClient(prev_code, p_snapshot_prev);
            ...
            else
            {
                ...
                MD_LOG_WARN("invalid data without last flag.");
            }
        }
...
SHFEMDQuoteSnapshot* MYShfeMDManager::GetDataCache(const std::string& code)
{
    ...
    if (it == code_index_.end())
    {
        ...
        {
            MD_LOG_WARN("code count beyond: %d", code_count_max);
    ...
    {
        MD_LOG_WARN("price position count beyond: %d", price_position_count_max);
...
SHFEMDQuoteSnapshot *MYShfeMDManager::PushDataToBuffer(const std::string &cur_code, MBLDataCollection::iterator &p)
{
    ...
    {
        // handle duplicate query datas
        if (p_data->buy_count == 0 || (p_data->buy_price[p_data->buy_count - 1] > p->field.Price))
        {
    ...
    {
        if (p_data->sell_count == 0 || (p_data->sell_price[p_data->sell_count - 1] < p->field.Price))
        {
...
void MYShfeMDManager::SendToClient(const std::string &code, SHFEMDQuoteSnapshot * const p_data)
{
    ...
    my_data.data_flag = 2;
    {
        _TTSpinLockGuard guard(depth_mutex_);
    ...
    // filter invalid data
    if (my_data.data_flag == 3 && (my_data.BidVolume1  > (int)my_data.buy_total_volume || my_data.AskVolume1 > (int)my_data.sell_total_volume))
    {
        my_data.data_flag = 1;
    }
    ...
    // reduce log
    if ((p_data->buy_count + p_data->sell_count) > 100)
    {
        MD_LOG_DEBUG("snapshot, code: %s, buy_count: %d, sell_count: %d", code.c_str(), p_data->buy_count, p_data->sell_count);
    }
