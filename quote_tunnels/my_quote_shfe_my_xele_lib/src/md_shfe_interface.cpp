...
void InitOnce()
{
    ...
    else
    {
        ...
        MD_LOG_INFO("start init quote library.");

        // initialize quote monitor
        qtm_init(TYPE_QUOTE);
        std::string qtm_name_id = "xele_shfe_" + boost::lexical_cast<std::string>(getpid());
        update_state(qtm_name_id.c_str(), TYPE_QUOTE, QtmState::INIT, GetDescriptionWithState(QtmState::INIT).c_str());
        MY_LOG_INFO("update_state: name: %s, State: %d, Description: %s.", qtm_name_id.c_str(), QtmState::INIT, GetDescriptionWithState(QtmState::INIT).c_str());
        s_have_init = true;
    }
}
...
MYQuoteData::MYQuoteData(const SubscribeContracts *subscribe_contracts, const std::string &provider_config_file)
{
    ...
    {
        MD_LOG_ERROR("no quote provider config file");
    }

    MD_LOG_INFO("create MYQuoteData object with configure file: %s", provider_config_file.c_str());
    ...
    {
        MD_LOG_ERROR("not support quote provider type(%d), please check config file.", provider_type);
    }
}
...
{
    // 连接服务
    MD_LOG_INFO("prepare to start shfe of my quote source data transfer");
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const SHFEQuote*)> quote_handler)
{
    ...
    {
        MD_LOG_ERROR("SHFEQuote handler function not match quote provider.");
    }
}
