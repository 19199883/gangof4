...
void FemasTunnel::RespondHandleThread(FemasTunnel *ptunnel)
{
    MyFemasTradeSpi * p_femas = static_cast<MyFemasTradeSpi *>(ptunnel->trade_inf_);
    if (!p_femas)
    {
    ...
    std::mutex &rsp_sync = p_femas->rsp_sync;
    std::condition_variable &rsp_con = p_femas->rsp_con;
...
void FemasTunnel::SendRespondAsync(int rsp_type, void *rsp)
{
    MyFemasTradeSpi * p_femas = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_femas)
    {
        TNL_LOG_ERROR("tunnel not init in RespondHandleThread.");
        return;
...
FemasTunnel::FemasTunnel(const std::string &provider_config_file)
{
    ...
    TNL_LOG_INFO("create FemasTunnel object with configure file: %s", cfg_file.c_str());
    ...
    if (provider_type == TunnelProviderType::PROVIDERTYPE_FEMAS)
    {
        InitInf(*lib_cfg_);
    }
    else
    ...
    // init respond thread
    std::thread rsp_thread(&FemasTunnel::RespondHandleThread, this);
    rsp_thread.detach();
}
...
void FemasTunnel::PlaceOrder(const T_PlaceOrder *pPlaceOrder)
{
    ...
    if (p_tunnel)
    {
        ...
        else
        {
            ...
            p_tunnel->femas_trade_context_.SaveSerialNoToOrderRef(order_ref,
            ...
            p_tunnel->femas_trade_context_.SaveOrderRefToStatus(order_ref,
    ...
    if (process_result != TUNNEL_ERR_CODE::RESULT_SUCCESS)
    {
        ...
        FEMASPacker::OrderRespond(process_result, pPlaceOrder->serial_no, "0", MY_TNL_OS_ERROR, *pOrderRespond);
...
void FemasTunnel::CancelOrder(const T_CancelOrder *pCancelOrder)
{
    ...
    if (p_tunnel)
    {
            ...
            || (process_result == TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT))
        {
            ...
            p_tunnel->femas_trade_context_.SaveSerialNoToOrderRef(order_ref,
                OriginalReqInfo(pCancelOrder->serial_no,
                    0, 0,
        ...
        && process_result != TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT)
    {
        ...
        FEMASPacker::CancelRespond(process_result, pCancelOrder->serial_no, "0", *cancle_order);
...
void FemasTunnel::QueryPosition(const T_QryPosition *pQryPosition)
{
    ...
    CUstpFtdcQryInvestorPositionField qry_param;
    memset(&qry_param, 0, sizeof(CUstpFtdcQryInvestorPositionField));
    ///经纪公司编号
    strncpy(qry_param.BrokerID, lib_cfg_->Logon_config().brokerid.c_str(), sizeof(TUstpFtdcBrokerIDType));
    ///用户代码
...
void FemasTunnel::QueryOrderDetail(const T_QryOrderDetail *pQryParam)
{
...
void FemasTunnel::QueryTradeDetail(const T_QryTradeDetail *pQryParam)
{
    ...
    CUstpFtdcQryTradeField qry_param;
    memset(&qry_param, 0, sizeof(CUstpFtdcQryTradeField));
    ///经纪公司编号
    strncpy(qry_param.BrokerID, lib_cfg_->Logon_config().brokerid.c_str(), sizeof(TUstpFtdcBrokerIDType));
    ///用户代码
    strncpy(qry_param.UserID, lib_cfg_->Logon_config().clientid.c_str(), sizeof(TUstpFtdcUserIDType));
...
void FemasTunnel::QueryContractInfo(const T_QryContractInfo *p)
{
...
//empty, femas can't support quote now 20150313
// 询价
void FemasTunnel::ReqForQuoteInsert(const T_ReqForQuote *p)
{
    LogUtil::OnReqForQuote(p, tunnel_info_);
}
///报价录入请求
void FemasTunnel::ReqQuoteInsert(const T_InsertQuote *p)
{
    LogUtil::OnInsertQuote(p, tunnel_info_);
}
///报价操作请求
void FemasTunnel::ReqQuoteAction(const T_CancelQuote *p)
{
    LogUtil::OnCancelQuote(p, tunnel_info_);
}

void FemasTunnel::SetCallbackHandler(std::function<void(const T_OrderRespond *)> callback_handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_CancelRespond *)> callback_handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_OrderReturn *)> callback_handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_TradeReturn *)> callback_handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_PositionReturn *)> handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_OrderDetailReturn *)> handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_TradeDetailReturn *)> handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_ContractInfoReturn *)> handler)
{
    MyFemasTradeSpi * p_tunnel = static_cast<MyFemasTradeSpi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void FemasTunnel::SetCallbackHandler(std::function<void(const T_RtnForQuote *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_RtnForQuote)");
}
void FemasTunnel::SetCallbackHandler(std::function<void(const T_RspOfReqForQuote *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_RspOfReqForQuote)");
}
void FemasTunnel::SetCallbackHandler(std::function<void(const T_InsertQuoteRespond *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_InsertQuoteRespond)");
}
void FemasTunnel::SetCallbackHandler(std::function<void(const T_CancelQuoteRespond *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_CancelQuoteRespond)");
}
void FemasTunnel::SetCallbackHandler(std::function<void(const T_QuoteReturn *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_QuoteReturn)");
}
void FemasTunnel::SetCallbackHandler(std::function<void(const T_QuoteTrade *)> handler)
{
...
{
    return new FemasTunnel(tunnel_config_file);
}
