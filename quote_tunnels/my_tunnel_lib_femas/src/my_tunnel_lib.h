...
class FemasTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    FemasTunnel(const std::string &ctp_config_file);
    virtual ~FemasTunnel();
...
private:
    // 禁止拷贝和赋值
    FemasTunnel(const FemasTunnel & other);
    FemasTunnel operator=(const FemasTunnel & other);
    ...
    static void RespondHandleThread(FemasTunnel *ptunnel);
};
