...
MYQuoteData::MYQuoteData(const SubscribeContracts *subscribe_contracts, const std::string &provider_config_file)
{
    ...
    if (provider_type == QuoteProviderType::PROVIDERTYPE_SEC_KMDS_CITICS)
    {
...
{
    // 连接服务
    MY_LOG_INFO("prepare to start kmds quote source data transfer");
    interface_ = new QuoteInterface_KMDS(subscribe_contracts, cfg);
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const TDF_MARKET_DATA_MY*)> quote_handler)
{
    if (interface_)
    {
        ((QuoteInterface_KMDS *)interface_)->SetQuoteDataHandler(quote_handler);
    }
    else
    {
        MY_LOG_ERROR("TDF_MARKET_DATA_MY quote handler function not match quote provider.");
    }
}
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const TDF_INDEX_DATA_MY*)> quote_handler)
{
    ...
    {
        MY_LOG_ERROR("TDF_INDEX_DATA_MY quote handler function not match quote provider.");
    }
}
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const T_OptionQuote*)> quote_handler)
{
    ...
    {
        MY_LOG_ERROR("T_OptionQuote quote handler function not match quote provider.");
    }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const T_OrderQueue*)> quote_handler)
{
    if (interface_)
    {
        ((QuoteInterface_KMDS *)interface_)->SetQuoteDataHandler(quote_handler);
    ...
    {
        MY_LOG_ERROR("T_OrderQueue quote handler function not match quote provider.");
    }
}
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const T_PerEntrust*)> quote_handler)
{
    ...
    {
        MY_LOG_ERROR("T_PerEntrust quote handler function not match quote provider.");
    }
}
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const T_PerBargain*)> quote_handler)
{
    ...
    {
        MY_LOG_ERROR("T_PerBargain quote handler function not match quote provider.");
    }
}

MYQuoteData::~MYQuoteData()
{
    if (interface_) delete ((QuoteInterface_KMDS *)interface_);
    qtm_finish();
}
