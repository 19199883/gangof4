...
public:
    ...
    inline static void FillContractInfo(T_ContractInfoReturn &rtn);
...
inline void ESUNNYFieldConvert::AddContractInfo(const TapAPITradeContractInfo& info)
{
    ...
    if (info.CallOrPutFlag1 != TAPI_CALLPUT_FLAG_NONE)
    {
    	contract.append(1, info.CallOrPutFlag1);
    	contract.append(info.StrikePrice1);
    }
    if (info.ContractNo2[0] != '\0')
    {
        ...
        if (info.CallOrPutFlag2 != TAPI_CALLPUT_FLAG_NONE) {
        	contract.append(1, info.CallOrPutFlag2);
        	contract.append(info.StrikePrice2);
        }
    }
...
inline void ESUNNYFieldConvert::FillContractInfo(T_ContractInfoReturn &rtn)
{
    std::lock_guard<std::mutex> lock(contract_mutex);

    ContractInfo item;

    for (auto it = contract_in_market.begin(); it != contract_in_market.end(); it++) {
    	memset(&item, 0, sizeof(item));
		if (it->second.CommodityType == TAPI_COMMODITY_TYPE_FUTURES
				|| it->second.CommodityType == TAPI_COMMODITY_TYPE_OPTION) {

			strcpy(item.TradeDate, my_cmn::GetCurrentDateString().c_str());

			std::string tmpdate, infodate(it->second.LastTradeDate);
			tmpdate.append(infodate.substr(0, 4));
			tmpdate.append(infodate.substr(5, 2));
			tmpdate.append(infodate.substr(8, 2));
			strcpy(item.ExpireDate, tmpdate.c_str());

			std::string stock;
			stock.append(it->second.CommodityNo);
			stock.append(it->second.ContractNo1);
			if (it->second.CommodityType == TAPI_COMMODITY_TYPE_OPTION) {
				stock.append(1, it->second.CallOrPutFlag1);
				stock.append(it->second.StrikePrice1);
			}
			strcpy(item.stock_code, stock.c_str());

			rtn.datas.push_back(item);
		}
	}
}
