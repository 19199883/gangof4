    ...
    char server_flag)
{
    ...
    {
    	 EsunnyOrderInfo *p;
    	p = new EsunnyOrderInfo(*po_tmp_);

        sn_to_order_.insert(std::make_pair(p->po.serial_no, p));
...
ESUNNYQuoteContext::ESUNNYQuoteContext()
{
    iq_tmp_ = NULL;
}

void ESUNNYQuoteContext::SaveQuoteInfo(TAPIUINT32 session_id, const EsunnyQuoteInfo *quote_info)
{
    std::lock_guard<std::mutex> lock(quote_mutex_);
    sn_to_quote_.insert(std::make_pair(quote_info->iq.serial_no, quote_info));
    bool insert_result = sessionid_to_quote_.insert(std::make_pair(session_id, quote_info)).second;
    if (!insert_result)
    {
        TNL_LOG_WARN("duplicate SessionID when InsertOrder, SessionID:%d.", session_id);
...
const EsunnyQuoteInfo * ESUNNYQuoteContext::GetQuoteInfoBySN(SerialNoType sn)
{
    std::lock_guard<std::mutex> lock(quote_mutex_);

    SNToEsunnyQuoteInfoMap::const_iterator cit = sn_to_quote_.find(sn);
    if (cit != sn_to_quote_.end())
    {
        return cit->second;
    }
...
const EsunnyQuoteInfo * ESUNNYQuoteContext::GetQuoteInfoByOrderNo(const std::string &quote_no)
{
    std::lock_guard<std::mutex> lock(quote_mutex_);

    OrderNoToEsunnyQuoteInfoMap::const_iterator cit = orderno_to_quote_.find(quote_no);
    if (cit != orderno_to_quote_.end())
    {
        return cit->second;
    }
...
const EsunnyQuoteInfo * ESUNNYQuoteContext::UpdateQuoteNoAndGetQuoteInfo(TAPIUINT32 session_id, const char *quote_no, char server_flag)
{
    std::lock_guard<std::mutex> lock(quote_mutex_);
    SessionIDToEsunnyQuoteInfoMap::const_iterator cit = sessionid_to_quote_.find(session_id);

    // 回报可能在报单函数返回前回来，因报单是串行处理的，此回报肯定是当前正在报的单产生，直接存储关联关系
    if (cit == sessionid_to_quote_.end() && iq_tmp_ != NULL)
    {
    	 EsunnyQuoteInfo *p;
    	p = new EsunnyQuoteInfo(*iq_tmp_);

        sn_to_quote_.insert(std::make_pair(p->iq.serial_no, p));
        cit = sessionid_to_quote_.insert(std::make_pair(session_id, p)).first;
    }

    if (cit != sessionid_to_quote_.end())
    {
        if (quote_no != NULL)
        {
            if (cit->second->order_no[0] == 0)
            {
                strncpy(cit->second->order_no, quote_no, sizeof(TAPISTR_20));
                orderno_to_quote_.insert(std::make_pair(std::string(quote_no), cit->second));
            }
            else if (strcmp(quote_no, cit->second->order_no) != 0)
            {
                TNL_LOG_ERROR("OrderNo of SessionID(%d) changed, from %s to %s.", session_id, cit->second->order_no, quote_no);
            }
        }
        cit->second->server_flag = server_flag;
...
void ESUNNYQuoteContext::SaveCancelOrForQuoteSnOfSessionID(TAPIUINT32 session_id, SerialNoType c_sn)
{
    std::lock_guard<std::mutex> lock(cancel_mutex_);
    sessionid_to_cancel_sn_.insert(std::make_pair(session_id, c_sn));
}

SerialNoType ESUNNYQuoteContext::GetSnBySessionID(TAPIUINT32 session_id)
{
    std::lock_guard<std::mutex> lock(cancel_mutex_);
