...
public:
    ...
    int ReqOrderInsert(const T_PlaceOrder *po)
    {
        ...
        try
        {
            ...
            {
                TNL_LOG_WARN(
                    "PlaceOrder can't find contract info to complete the order");
                return ret;
            }
            ...
            TAPIUINT32 session_id;
            {
                ...
                {
                    esunny_trade_context_.SaveOrderInfo(session_id,
                        new EsunnyOrderInfo(*po));
                }
            }

            TNL_LOG_DEBUG(
                "InsertOrder - return:%d, session_id:%d, serial_no:%ld",
                ret, session_id, po->serial_no);
    ...
    int ReqOrderAction(const T_CancelOrder *pc)
    {
        ...
        try
        {
            ...
            const EsunnyOrderInfo * org_order_info =
                esunny_trade_context_.GetOrderInfoBySN(pc->org_serial_no);
            if (!org_order_info)
            {
            ...
            if (ret == TAPIERROR_SUCCEED)
            {
                esunny_trade_context_.SaveCancelSnOfSessionID(session_id,
                    pc->serial_no);
                T_CancelRespond rsp;
            ...
            TNL_LOG_DEBUG(
                "CancelOrder - return:%d, session_id:%d, cancel_sn:%ld, order_sn:%ld",
                ret, session_id, pc->serial_no, pc->org_serial_no);
    ...
    int QryContractInfo(const T_QryContractInfo *pQryParam)
    {
        ...
        if (HaveFinishQueryOrders()) {
        	...
        	ESUNNYFieldConvert::FillContractInfo(rtn);
        	rtn.error_no = 0;
    ...
    //做市商操作请求
    int ReqForQuoteInsert(const T_ReqForQuote *p)
    {
        if (!TunnelIsReady())
        {
            TNL_LOG_WARN("ReqOrderInsert when tunnel not ready");
        ...
        try
        {
            TapAPINewOrder p_req;
            ESUNNYPacker::QuoteRequest(cfg_, p, p_req);
            TAPIUINT32 session_id;
            int ret;
            {
                std::lock_guard<std::mutex> lock(api_mutex_);
                ret = api_->InsertOrder(&session_id, &p_req);

                esunny_quote_context_.SaveCancelOrForQuoteSnOfSessionID(
                    session_id, p->serial_no);
            }

            if (ret == TAPIERROR_SUCCEED)
            {
                T_RspOfReqForQuote rsp;
                ESUNNYPacker::QuoteRequestRespond(p, ret, rsp);
            ...
            TNL_LOG_INFO("ReqForQuoteInsert - session_id:%d, return:%d",
                session_id, ret);
            return ConvertErrorCode(ret);
        }
        catch (...)
    ...
    int ReqQuoteInsert(const T_InsertQuote *pReq)
    {
        if (!TunnelIsReady())
        {
        ...
        try
        {
            ...
            bool convert_res = ESUNNYPacker::QuoteInsertRequest(cfg_, pReq,
                new_or);
            ...
            //TNL_LOG_DEBUG(ESUNNYDatatypeFormater::ToString(&new_or).c_str());
            TAPIUINT32 session_id;
            int ret;
            {
                std::lock_guard<std::mutex> lock(api_mutex_);
                ret = api_->InsertOrder(&session_id, &new_or);

                if (ret == TAPIERROR_SUCCEED)
                {
                    EsunnyQuoteInfo *order_info = new EsunnyQuoteInfo(*pReq);
                    esunny_quote_context_.SaveQuoteInfo(session_id, order_info);

                    T_InsertQuoteRespond rsp;
                    ESUNNYPacker::QuoteInsertRespond(pReq, ret, rsp);
            ...
            TNL_LOG_INFO("ReqQuoteInsert - session_id:%d, return:%d",
                session_id, ret);
            return ConvertErrorCode(ret);
        }
        catch (...)
    ...
    int ReqQuoteAction(const T_CancelQuote *pReq)
    {
        if (!TunnelIsReady())
        {
        ...
        {
            // create cancel request object
            const EsunnyQuoteInfo * org_order_info =
                esunny_quote_context_.GetQuoteInfoBySN(pReq->org_serial_no);
            if (!org_order_info)
            {
                TNL_LOG_WARN("ReqQuoteAction can't get original order by sn");
                return ret;
            }
            TapAPIOrderCancelReq cancel_req;
            ESUNNYPacker::CancelRequest(org_order_info, cancel_req);
            ...
            TAPIUINT32 session_id;
            {
                ...
                {
                    esunny_quote_context_.SaveCancelOrForQuoteSnOfSessionID(
                        session_id, pReq->serial_no);

                    T_CancelQuoteRespond rsp;
                    ESUNNYPacker::QuoteActionRespond(pReq, ret, rsp);
            ...
            TNL_LOG_DEBUG(
                "ReqQuoteAction - return:%d, session_id:%d, cancel_sn:%ld, order_sn:%ld",
                ret, session_id, pReq->serial_no, pReq->org_serial_no);
            //TNL_LOG_DEBUG("ReqQuoteAction - return:%d, session_id:%d - %s", ret, session_id, ESUNNYDatatypeFormater::ToString(&cancel_req).c_str());
        }
        catch (...)
        {
...
private:
    ...
    ESUNNYQuoteContext esunny_quote_context_;
    ...
    std::function<void(const T_RtnForQuote *)> RtnOfReqForQuoteHandler_;
    ...
    std::function<void(const T_QuoteReturn *)> RtnOfInsertQuoteHandler_;
    std::function<void(const T_QuoteTrade *)> RtnOfCancelQuoteHandler_;
