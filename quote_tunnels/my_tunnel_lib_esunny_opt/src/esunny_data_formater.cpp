...
std::string ESUNNYDatatypeFormater::ToString(const TapAPIUpperChannelInfo *p)
{
	char buf[10240];

	if (p) {
		snprintf(buf, sizeof(buf), "structName=TapAPIUpperChannelInfo\n"
				"\tUpperChannelNo=%s\n"
				"\tUpperChannelName=%s\n"
				"\tUpperNo=%s\n"
				"\tUpperUserNo=%s\n",
	...
	{
		snprintf(buf, sizeof(buf), "structName=TapAPIUpperChannelInfo <null>");
	}

	return buf;
}

std::string ESUNNYDatatypeFormater::ToString(const TapAPIReqQuoteNotice *p)
{
	char buf[10240];

	if (p) {
		snprintf(buf, sizeof(buf), "structName=TapAPIReqQuoteNotice\n"
				"\tExchangeNo=%s\n"
				"\tCommodityType=%c\n"
				"\tCommodityNo=%s\n"
				"\tContractNo=%s\n"
	...
	{
		snprintf(buf, sizeof(buf), "structName=TapAPIReqQuoteNotice <null>");
	}
