#ifndef _MY_TUNNEL_LIB_CTP_H_
#define _MY_TUNNEL_LIB_CTP_H_
...
class CtpTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    CtpTunnel(const std::string &ctp_config_file);
    virtual ~CtpTunnel();
...
private:
    // 禁止拷贝和赋值
    CtpTunnel(const CtpTunnel & other);
    CtpTunnel operator=(const CtpTunnel & other);
    ...
    void QueryPositionBeforeClose(const std::string &time);
    ...
    static void RespondHandleThread(CtpTunnel *ptunnel);
};
