...
MYQuoteData::MYQuoteData(const SubscribeContracts *subscribe_contracts, const std::string &provider_config_file)
{
    ...
    if (provider_type == QuoteProviderType::PROVIDERTYPE_TDF)
    {
...
{
    // 连接服务
    MY_LOG_INFO("prepare to start zeusing udp quote source data transfer");
    interface_ = new QuoteInterface_TDF(subscribe_contracts, cfg);
...
void MYQuoteData::SetQuoteDataHandler(boost::function<void(const TDF_MARKET_DATA_MY*)> quote_handler)
{
    if (interface_)
    {
        ((QuoteInterface_TDF *)interface_)->SetQuoteDataHandler(quote_handler);
    }
    else
    {
...
{
    if (interface_) delete ((QuoteInterface_TDF *)interface_);
    qtm_finish();
}
