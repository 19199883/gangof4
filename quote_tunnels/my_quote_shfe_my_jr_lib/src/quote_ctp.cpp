    ...
    : cfg_(cfg), p_save_(NULL)
{
    ...
    sprintf(qtm_name_, "shfe_jr_%s_%u", cfg.Logon_config().account.c_str(), getpid());
    QuoteUpdateState(qtm_name_, QtmState::INIT);

    p_save_ = new QuoteDataSave<CDepthMarketDataField>(cfg_, qtm_name_, "quote_level1", SHFE_EX_QUOTE_TYPE);
    ...
    sub_count_ = 0;
    {
        ...
        int i = 0;
        BOOST_FOREACH(const MYSecurityCollection::value_type &value, MYCTPTradeInterface::securities_exchcode)
        {
...
//	MonitorClient::instance()->SendMonitorMessage('S', 1, "quote server disconnected");
}
...
void MYCTPDataHandler::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *p)
{
    try
    {
        ...
        // transfer all, don't need filter
