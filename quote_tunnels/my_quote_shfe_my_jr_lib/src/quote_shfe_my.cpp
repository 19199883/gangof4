        ...
        p_mbl_handler_(NULL), my_shfe_md_inf_(cfg)
{
    ...
    ctp_inf_ = new MYCTPDataHandler(NULL, cfg_);
    ctp_inf_->SetQuoteDataHandler(boost::bind(&QuoteInterface_MY_SHFE_MD::ShfeDepthMarketDataHandler, this, _1));
}
...
#define RECV_BUFFER_LEN (56 * 4000)
void QuoteInterface_MY_SHFE_MD::ShfeMBLHandler()
{
    ...
    while (running_flag_)
    {
        ...
        recv_len = recvfrom(udp_fd, recv_buf + data_len, RECV_BUFFER_LEN - data_len, 0, (sockaddr *) &fromAddr,
            (socklen_t *) &nFromLen);
        ...
        data_len += recv_len;
        if (data_len < sizeof(SHFEQuote))
        {
            MY_LOG_ERROR("recv buffer length is less than struct");
            continue;
        }

        // data handle
        SHFEQuote *p = (SHFEQuote *) recv_buf;
        {
            timeval t;
            gettimeofday(&t, NULL);
            do
            {
                if (shfe_deep_data_handler_
                    && (subscribe_contracts_.empty()
                        || subscribe_contracts_.find(p->field.InstrumentID) != subscribe_contracts_.end()))
                {
                    shfe_deep_data_handler_(p);
                }
                my_shfe_md_inf_.OnMBLData(&p->field, p->isLast);
                p_shfe_deep_save_->OnQuoteData(t.tv_sec * 1000000 + t.tv_usec, p);

                data_len -= sizeof(SHFEQuote);
                ++p;
            }
            while (data_len >= sizeof(SHFEQuote));
        }

        // left data, move to head
        if (data_len > 0)
        {
            memcpy(recv_buf, p, data_len);
        }
    }
}
