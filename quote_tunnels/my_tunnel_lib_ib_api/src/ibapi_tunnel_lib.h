...
class IBAPITunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    IBAPITunnel(const std::string &ctp_config_file);
    virtual ~IBAPITunnel();
...
private:
    // 禁止拷贝和赋值
    IBAPITunnel(const IBAPITunnel & other);
    IBAPITunnel operator=(const IBAPITunnel & other);
    ...
    static void RespondHandleThread(IBAPITunnel *ptunnel);
};
