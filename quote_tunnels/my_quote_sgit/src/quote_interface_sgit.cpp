...
MYQuoteData::MYQuoteData(const SubscribeContracts *subscribe_contracts, const std::string &provider_config_file)
{
    ...
    if (provider_type == QuoteProviderType::PROVIDERTYPE_SGIT)
    {
...
{
    // 连接服务
    MY_LOG_INFO("prepare to start - sgit - quote source data transfer");
    interface_ = new SgitDataHandler(subscribe_contracts, cfg);

    return true;
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const struct SGIT_Depth_Market_Data_Field *)> quote_handler)
{
    if (interface_)
    {
        ((SgitDataHandler *)interface_)->SetQuoteDataHandler(quote_handler);
    }
    else
    {
        MY_LOG_ERROR("sgit quote handler function not match quote provider.");
    }
}
...
MYQuoteData::~MYQuoteData()
{
    if (interface_) 
    {
        delete ((SgitDataHandler *)interface_);
    }
}
