...
class RemTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    RemTunnel(const std::string &ctp_config_file);
    virtual ~RemTunnel();
...
private:
    // 禁止拷贝和赋值
    RemTunnel(const RemTunnel & other);
    RemTunnel operator=(const RemTunnel & other);
    ...
    static void RespondHandleThread(RemTunnel *ptunnel);
};
