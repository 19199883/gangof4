...
//=============================================================================
/**
 *	\addtogroup G_DATATYPE_Q_TAPIBUKETDATEFLAG	交易时段日期标记
 *	@{
 */
//=============================================================================

//! 交易时段日期标记
typedef TAPICHAR			TAPIBucketDateFlag;
//! 前一日时间
const TAPIBucketDateFlag	TAPI_TIMEBUCKETDATEFLAG_PREDAY	='0';
//! 当日
const TAPIBucketDateFlag	TAPI_TIMEBUCKETDATEFLAG_CURDAY	='1';
//! 下一日
const TAPIBucketDateFlag	TAPI_TIMEBUCKETDATEFLAG_NEXTDAY	='2';
...
//=============================================================================
/**
 *	\addtogroup G_DATATYPE_Q_HISQUOTETYPE	历史行情类型
 *	@{
 */
//=============================================================================
//! 历史行情类型
typedef TAPICHAR			TAPIHisQuoteType;
//! 分钟线
const TAPIHisQuoteType	TAPI_HISQUOTETYPE_MINUTE		= '0';
//! 日线
const TAPIHisQuoteType	TAPI_HISQUOTETYPE_DAY			= '1';
/** @}*/
...
//! 历史行情查询请求结构
struct TapAPIHisQuoteQryReq
{
	TapAPIContract		Contract;								///< 合约
	TAPIHisQuoteType	HisQuoteType;							///< K线类型
	TAPIDATETIME		HisQuoteEndTime;						///< K线结束时间
	TAPIUINT64			HisQuoteCandleQryCount;					///< K线根数
};
...
//! 交易时段
struct TapAPITradingTimeBucket
{
	TAPIINT32				TradingTimeBucketID;                    ///< 交易时段编号
	TAPITIME				TimeBucketBeginTime;                    ///< 交易时段开始时间
	TAPICHAR				TradingState;                           ///< 交易状态
	TAPIYNFLAG				IsDST;                                  ///< 是否夏令时,'Y'为是,'N'为否
	TAPIBucketDateFlag		DateFlag;								///< 交易日标志
};

//! 交易时段查询应答结构
struct TapAPITimeBucketOfCommodityInfo
{
	TapAPICommodity				Commodity;							///< 品种
	TAPIUINT32					Count;								///< 交易时段个数
	TapAPITradingTimeBucket*	TimeBuckets;						///< 交易时段数组首地址
};
...
//! 历史行情
struct TapAPIHisQuote
{
	TAPIHisQuoteType			HisQuoteType;					///< 历史行情类型
	TapAPIContract				Contract;						///< 合约
	TAPIDTSTAMP					DateTimeStamp;					///< 时间戳
	TAPIDATE					TradeDate;						///< 交易日
	TAPIUINT32					KLineIndex;						///< K线索引
	TAPIQPRICE					QOpeningPrice;					///< 开盘价
	TAPIQPRICE					QLastPrice;						///< 最新价
	TAPIQPRICE					QHighPrice;						///< 最高价
	TAPIQPRICE					QLowPrice;						///< 最低价
...
//! 历史行情查询应答结构
struct TapAPIHisQuoteQryRsp
{
	TAPIUINT32					Count;							///< 历史行情个数
	TapAPIHisQuote*				HisQuotes;						///< 历史行情数组首地址
};
