...
std::atomic_int X1Tunnel::counter_(0);
...
void X1Tunnel::RespondHandleThread(X1Tunnel *ptunnel)
{
    MYX1Spi * p_xspeed = static_cast<MYX1Spi *>(ptunnel->trade_inf_);
    if (!p_xspeed)
    {
...
void X1Tunnel::SendRespondAsync(int rsp_type, void *rsp)
{
    MYX1Spi * p_xspeed = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_xspeed)
    {
        TNL_LOG_ERROR("tunnel not init in RespondHandleThread.");
        exit(0);
...
std::string X1Tunnel::GetClientID()
{
    return tunnel_info_.account;
}

X1Tunnel::X1Tunnel(const std::string &provider_config_file)
{
    trade_inf_ = NULL;
    exchange_code_ = ' ';
    InitOnce();
    ...
    // init respond thread
    std::thread rsp_thread(&X1Tunnel::RespondHandleThread, this);
    rsp_thread.detach();
}

bool X1Tunnel::InitInf(const TunnelConfigData &cfg)
{
    // 连接服务
...
void X1Tunnel::PlaceOrder(const T_PlaceOrder *pPlaceOrder)
{
#ifdef PROBE_COST
    char tag_name[32];
    ...
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
...
#endif
            CX1FtdcInsertOrderField counter_req;
            X1Packer::OrderRequest(*lib_cfg_, pPlaceOrder, order_ref, counter_req);
            p_tunnel->xspeed_trade_context_.SaveOrderInfo(order_ref, X1OrderInfo(exchange_code_, pPlaceOrder));
#ifdef PROBE_COST
    ...
    if (process_result != TUNNEL_ERR_CODE::RESULT_SUCCESS)
    {
        ...
        X1Packer::OrderRespond(process_result, pPlaceOrder->serial_no, 0, MY_TNL_OS_ERROR, *pOrderRespond);
...
void X1Tunnel::CancelOrder(const T_CancelOrder *pCancelOrder)
{
#ifdef PROBE_COST
    char tag_name[32];
    memset(tag_name, 0, sizeof(tag_name));
    ...
    TIME_PROBER_BEGIN(cancel_order_count*10+4);
			...
			|| (process_result == TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT))
        {
            ...
            CX1FtdcCancelOrderField counter_req;
            X1Packer::CancelRequest(*lib_cfg_, pCancelOrder, org_order_ref, counter_req);
        ...
        && process_result != TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT)
    {
        ...
        X1Packer::CancelRespond(process_result, pCancelOrder->serial_no, 0, *cancle_order);
...
void X1Tunnel::QueryPosition(const T_QryPosition *pQryPosition)
{
    ...
    CX1FtdcQryPositionDetailField qry_param;
    memset(&qry_param, 0, sizeof(CX1FtdcQryPositionDetailField));
    strncpy(qry_param.AccountID, tunnel_info_.account.c_str(), sizeof(TX1FtdcAccountIDType));
...
void X1Tunnel::QueryOrderDetail(const T_QryOrderDetail *pQryParam)
{
    ...
    CX1FtdcQryOrderField qry_param;
    memset(&qry_param, 0, sizeof(CX1FtdcQryOrderField));
    strncpy(qry_param.AccountID, tunnel_info_.account.c_str(), sizeof(TX1FtdcAccountIDType));
...
void X1Tunnel::QueryTradeDetail(const T_QryTradeDetail *pQryParam)
{
    ...
    CX1FtdcQryMatchField qry_param;
    memset(&qry_param, 0, sizeof(CX1FtdcQryMatchField));
    strncpy(qry_param.AccountID, tunnel_info_.account.c_str(), sizeof(TX1FtdcAccountIDType));
...
void X1Tunnel::QueryContractInfo(const T_QryContractInfo *p)
{
...
void X1Tunnel::ReqForQuoteInsert(const T_ReqForQuote *p)
{
...
///报价录入请求
void X1Tunnel::ReqQuoteInsert(const T_InsertQuote *p)
{
    ...
    X1Packer::QuoteRespond(process_result, p->serial_no, 0, process_result, *rsp);
...
///报价操作请求
void X1Tunnel::ReqQuoteAction(const T_CancelQuote *p)
{
    ...
    X1Packer::CancelQuoteRespond(process_result, p->serial_no, p->entrust_no, *cancel_quote_rsp);
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_OrderRespond *)> callback_handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_CancelRespond *)> callback_handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_OrderReturn *)> callback_handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_TradeReturn *)> callback_handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_PositionReturn *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_OrderDetailReturn *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_TradeDetailReturn *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_ContractInfoReturn *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_RtnForQuote *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_RspOfReqForQuote *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_InsertQuoteRespond *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_CancelQuoteRespond *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_QuoteReturn *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
void X1Tunnel::SetCallbackHandler(std::function<void(const T_QuoteTrade *)> handler)
{
    MYX1Spi * p_tunnel = static_cast<MYX1Spi *>(trade_inf_);
    if (!p_tunnel)
    {
...
{
    return new X1Tunnel(tunnel_config_file);
}
