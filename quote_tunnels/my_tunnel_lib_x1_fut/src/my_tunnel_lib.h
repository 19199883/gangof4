...
class X1Tunnel: public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    X1Tunnel(const std::string &ctp_config_file);
    virtual ~X1Tunnel();
...
private:
    // 禁止拷贝和赋值
    X1Tunnel(const X1Tunnel & other);
    X1Tunnel operator=(const X1Tunnel & other);
    ...
    static void RespondHandleThread(X1Tunnel *ptunnel);
};
