...
//
#ifndef X1_TRADE_CONTEXT_H_
#define X1_TRADE_CONTEXT_H_
...
struct X1OrderInfo
{
    ...
    X1OrderInfo(char exch_code, const T_PlaceOrder *p)
    {
        serial_no = p->serial_no;
        memcpy(stock_code, p->stock_code, sizeof(stock_code));
...
struct X1QuoteInfo
{
    ...
    X1QuoteInfo(const T_InsertQuote &d)
...
typedef std::map<OrderRefDataType, X1OrderInfo> OrderIDToOrderInfoMap;
typedef std::map<OrderRefDataType, X1QuoteInfo> OrderIDToQuoteInfoMap;

class X1TradeContext
{
public:
    X1TradeContext();
    ...
    // 报单引用和报单原始信息的映射
    void SaveOrderInfo(OrderRefDataType order_id, const X1OrderInfo &order_info);
    const X1OrderInfo * GetOrderInfoByLocalID(OrderRefDataType local_id);
    ...
    void SaveQuoteInfo(OrderRefDataType quote_ref, const X1QuoteInfo &quote_info);
    const X1QuoteInfo * GetQuoteInfoByOrderRef(OrderRefDataType order_ref);
