...
class ZeusingTunnel: public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    ZeusingTunnel(const std::string &config_file);
    virtual ~ZeusingTunnel();
...
private:
    // 禁止拷贝和赋值
    ZeusingTunnel(const ZeusingTunnel & other);
    ZeusingTunnel operator=(const ZeusingTunnel & other);
    ...
    static void RespondHandleThread(ZeusingTunnel *ptunnel);
};
