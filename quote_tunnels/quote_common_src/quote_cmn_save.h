...
{
public:
    QuoteDataSave(const ConfigData &cfg, const std::string &qtm_name, const std::string &file_name_prefix, int data_type, bool monitor = true)
        : cfg_(cfg), monitor_(monitor), qtm_name_(qtm_name)
    {
    ...
    void SaveImp()
    {
        ...
        while (running_flag_)
        {
            ...
            try
            {
                if (monitor_)
                {
                    MYUTIL_UpdateQuoteTime(qtm_name_, datas_t);
                }
