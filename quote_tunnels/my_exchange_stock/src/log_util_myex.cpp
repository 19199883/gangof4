...
char *MYExchangeLogUtil::shm_addr = NULL;
...
static std::string ToFormatedDatetime(long long t)
{
    ...
    strftime(tm_buff, 32, "%Y-%m-%d %T", localtime(&loc_t_sec));
    return tm_buff;
}
...
std::thread * MYExchangeLogUtil::save_thread = NULL;
std::string MYExchangeLogUtil::file_name_;
std::ofstream MYExchangeLogUtil::log_file_;
std::ofstream MYExchangeLogUtil::inner_log_file_;

void MYExchangeLogUtil::Start(const std::string &log_file_name, int shm_key)
{
    if (s_init_flag)
    {
        return;
...
//        EX_LOG_INFO("address of share memory: %p", shm_addr);
//    }
//    else
//    {
//        EX_LOG_INFO("close inner quote source feature");
//    }

    // 文件存储
    CreateFileHandle();

    save_thread = new std::thread(&MYExchangeLogUtil::SaveImp);
    s_init_flag = true;
    EX_LOG_INFO("create log save file thread");
}

void MYExchangeLogUtil::Stop()
{
    EX_LOG_INFO("stop log thread");
    if (!save_thread)
    {
        return;
    }
...
void MYExchangeLogUtil::SaveImp()
{
    ...
    EX_LOG_INFO("enter log thread, s_run_flag: %d", s_run_flag);
    while (s_run_flag)
    {
        // 将数据交换到本地
        {
...
long long MYExchangeLogUtil::GetTimeFromEpoch()
{
...
long long MYExchangeLogUtil::GetTimeAndPublish(int data_type, const void *pdata, std::size_t data_size)
{
...
void MYExchangeLogUtil::OnPlaceOrder(const T_PlaceOrder* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    place_orders.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnCancelOrder(const T_CancelOrder* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    // cache for write to file
    cancel_orders.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnOrderRespond(const T_OrderRespond* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    // cache for write to file
    order_responds.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnCancelRespond(const T_CancelRespond* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    // cache for write to file
    cancel_responds.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnOrderReturn(const T_OrderReturn* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    // cache for write to file
    order_returns.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnTradeReturn(const T_TradeReturn* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    ...
    // cache for write to file
    trade_returns.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQryPosition(const T_QryPosition* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    qry_position.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnPositionRtn(const T_PositionReturn *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    pos_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQryOrderDetail(const T_QryOrderDetail *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    qry_order_detail.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnOrderDetailRtn(const T_OrderDetailReturn *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    order_detail_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQryTradeDetail(const T_QryTradeDetail *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    qry_trade_detail.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnTradeDetailRtn(const T_TradeDetailReturn *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    trade_detail_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQryContractInfo(const T_QryContractInfo *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    qry_contract.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnContractInfoRtn(const T_ContractInfoReturn *p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    contract_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::CreateFileHandle()
{
    //Create Directory
    char cur_path[256];
...
std::string MYExchangeLogUtil::GetLocalVPNIP()
{
...
struct NoTypeDataAndTime
{
    ...
    NoTypeDataAndTime(long long t, char c, const void *d, const std::string &acc)
        : time(t), type(c), p_data(d), account(acc)
    {
    }
...
void MYExchangeLogUtil::OnReqForQuote(const T_ReqForQuote* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    req_forquote.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnInsertQuote(const T_InsertQuote* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    insert_quote.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnCancelQuote(const T_CancelQuote* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    cancel_quote.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnRspOfReqForQuote(const T_RspOfReqForQuote* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    rsp_req_forquote.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnRtnForQuote(const T_RtnForQuote* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    rtn_forquote.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnInsertQuoteRespond(const T_InsertQuoteRespond* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    insert_quote_rsp.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnCancelQuoteRespond(const T_CancelQuoteRespond* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    cancel_quote_rsp.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQuoteReturn(const T_QuoteReturn* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    quote_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::OnQuoteTrade(const T_QuoteTrade* p, const std::string &account)
{
    lock_guard<mutex> lock(log_sync);
    long long t = GetTimeFromEpoch();
    quote_trade_rtn.push_back(std::make_tuple(t, *p, account));
}

void MYExchangeLogUtil::SaveAll(const OrderAndTimes &place_orders_t,
    ...
    )
{
    ...
    for (NoTypeDataAndTimeCollection::const_iterator p = all_datas.begin(); p != all_datas.end(); ++p)
    {
        switch (p->type)
        {
            case '1':
                {
                log_file_ << ToString(p->time, (const T_PlaceOrder *) p->p_data, p->account) << std::endl;
                break;
            }
            case '2':
                {
                log_file_ << ToString(p->time, (const T_CancelOrder *) p->p_data, p->account) << std::endl;
                break;
            }
            case '3':
                {
                log_file_ << ToString(p->time, (const T_OrderRespond *) p->p_data, p->account) << std::endl;
                break;
            }
            case '4':
                {
                log_file_ << ToString(p->time, (const T_CancelRespond *) p->p_data, p->account)
                    << std::endl;
                break;
            }
            case '5':
                {
                log_file_ << ToString(p->time, (const T_OrderReturn *) p->p_data, p->account) << std::endl;
                break;
            }
            case '6':
                {
                log_file_ << ToString(p->time, (const T_TradeReturn *) p->p_data, p->account) << std::endl;
                break;
            ...
            case '7':
                {
                inner_log_file_ << ToString(p->time, (const T_QryPosition *) p->p_data, p->account) << std::endl;
                break;
            }
            case '8':
                {
                inner_log_file_ << ToString(p->time, (const T_PositionReturn *) p->p_data, p->account) << std::endl;
                break;
            }
            case '9':
                {
                inner_log_file_ << ToString(p->time, (const T_QryOrderDetail *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'a':
                {
                inner_log_file_ << ToString(p->time, (const T_OrderDetailReturn *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'b':
                {
                inner_log_file_ << ToString(p->time, (const T_QryTradeDetail *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'c':
                {
                inner_log_file_ << ToString(p->time, (const T_TradeDetailReturn *) p->p_data, p->account) << std::endl;
                break;
            }

            case 'd':
                {
                log_file_ << ToString(p->time, (const T_ReqForQuote *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'e':
                {
                log_file_ << ToString(p->time, (const T_InsertQuote *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'f':
                {
                log_file_ << ToString(p->time, (const T_CancelQuote *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'g':
                {
                log_file_ << ToString(p->time, (const T_RspOfReqForQuote *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'h':
                {
                log_file_ << ToString(p->time, (const T_RtnForQuote *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'i':
                {
                log_file_ << ToString(p->time, (const T_InsertQuoteRespond *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'j':
                {
                log_file_ << ToString(p->time, (const T_CancelQuoteRespond *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'k':
                {
                log_file_ << ToString(p->time, (const T_QuoteReturn *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'l':
                {
                log_file_ << ToString(p->time, (const T_QuoteTrade *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'm':
                {
                inner_log_file_ << ToString(p->time, (const T_QryContractInfo *) p->p_data, p->account) << std::endl;
                break;
            }
            case 'n':
                {
                inner_log_file_ << ToString(p->time, (const T_ContractInfoReturn *) p->p_data, p->account) << std::endl;
                break;
            }
            default:
                {
                EX_LOG_ERROR("unknown data type of log data, type: %d", p->type);
                break;
            }
        }
    }
