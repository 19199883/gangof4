...
class LTSTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    LTSTunnel(const std::string &ctp_config_file);
    virtual ~LTSTunnel();
...
private:
    // 禁止拷贝和赋值
    LTSTunnel(const LTSTunnel & other);
    LTSTunnel operator=(const LTSTunnel & other);
    ...
    static void RespondHandleThread(LTSTunnel *ptunnel);
};
