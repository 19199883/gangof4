...
void LTSTunnel::RespondHandleThread(LTSTunnel *ptunnel)
{
    LTSTradeInf * p_lts = static_cast<LTSTradeInf *>(ptunnel->trade_inf_);
    if (!p_lts)
    {
    ...
    std::mutex &rsp_sync = p_lts->rsp_sync;
    std::condition_variable &rsp_con = p_lts->rsp_con;
...
void LTSTunnel::SendRespondAsync(int rsp_type, void *rsp)
{
    LTSTradeInf * p_lts = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_lts)
    {
        TNL_LOG_ERROR("tunnel not init in RespondHandleThread.");
        return;
...
LTSTunnel::LTSTunnel(const std::string &provider_config_file)
{
    trade_inf_ = NULL;
    InitOnce();

    std::string cfg_file("my_trade_tunnel_lts.xml");
    if (!provider_config_file.empty())
    {
        cfg_file = provider_config_file;
    }

    TNL_LOG_INFO("create LTSTunnel object with configure file: %s", cfg_file.c_str());
    ...
    sprintf(qtm_tmp_name, "lts_%s_%d_%u", tunnel_info_.account.c_str(), lib_cfg_->App_cfg().orderref_prefix_id, getpid());
    tunnel_info_.qtm_name = qtm_tmp_name;
    TunnelUpdateState(tunnel_info_.qtm_name.c_str(), QtmState::INIT);

    // init tunnel log object
    LogUtil::Start("my_tunnel_lib_lts", lib_cfg_->App_cfg().share_memory_key);

    int provider_type = lib_cfg_->App_cfg().provider_type;
    if (provider_type == TunnelProviderType::PROVIDERTYPE_LTS)
    {
        InitInf(*lib_cfg_);
    }
    else
    ...
    // init respond thread
    std::thread rsp_thread(&LTSTunnel::RespondHandleThread, this);
    rsp_thread.detach();
}

std::string LTSTunnel::GetClientID()
{
    return tunnel_info_.account;
}

bool LTSTunnel::InitInf(const TunnelConfigData &cfg)
{
    // 连接服务
    TNL_LOG_INFO("prepare to start lts tunnel server.");
    ...
    trade_inf_ = new LTSTradeInf(cfg);
    return true;
}

void LTSTunnel::PlaceOrder(const T_PlaceOrder *pPlaceOrder)
{
    int process_result = TUNNEL_ERR_CODE::RESULT_FAIL;
    LogUtil::OnPlaceOrder(pPlaceOrder, tunnel_info_);
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    ...
    if (p_tunnel)
    {
        ...
        OrderRefDataType order_ref = p_tunnel->lts_trade_context_.GetNewOrderRef();
        TNL_LOG_DEBUG("serial_no: %ld map order_ref: %lld", pPlaceOrder->serial_no, order_ref);
        int request_id = p_tunnel->lts_trade_context_.GetRequestID();

        process_result = ComplianceCheck_TryReqOrderInsert(
            tunnel_info_.account.c_str(),
            order_ref,
            pPlaceOrder->exchange_type,
        ...
        else
        {
            ...
            CSecurityFtdcInputOrderField counter_req;
            LTSPacker::OrderRequest(*lib_cfg_, pPlaceOrder, order_ref, counter_req);
            p_tunnel->lts_trade_context_.SaveSerialNoToOrderRef(order_ref,
                OriginalReqInfo(pPlaceOrder->serial_no,
                    p_tunnel->Front_id(), p_tunnel->Session_id(),
                    pPlaceOrder->exchange_type, pPlaceOrder->direction, pPlaceOrder->speculator, pPlaceOrder->open_close,
                    pPlaceOrder->order_type,
    ...
    if (process_result != TUNNEL_ERR_CODE::RESULT_SUCCESS)
    {
        ...
        LTSPacker::OrderRespond(process_result, pPlaceOrder->serial_no, 0, *pOrderRespond);
...
void LTSTunnel::CancelOrder(const T_CancelOrder *pCancelOrder)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    ...
    if (p_tunnel)
    {
        ...
        int request_id = p_tunnel->lts_trade_context_.GetRequestID();
        OrderRefDataType org_order_ref = p_tunnel->lts_trade_context_.GetOrderRefBySerialNo(pCancelOrder->org_serial_no);
        const OriginalReqInfo * org_order_info = p_tunnel->lts_trade_context_.GetOrderInfoByOrderRef(org_order_ref);
            ...
            || (process_result == TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT))
        {
            ...
            CSecurityFtdcInputOrderActionField counter_req;
            LTSPacker::CancelRequest(*lib_cfg_, pCancelOrder, order_ref, org_order_ref, org_order_info,
                counter_req);

            char cancel_sn[64];
            sprintf(cancel_sn, "%ld", pCancelOrder->serial_no);
            p_tunnel->lts_trade_context_.UpdateCancelOrderRef(org_order_ref, cancel_sn);
        ...
        && process_result != TUNNEL_ERR_CODE::CANCEL_EQUAL_LIMIT)
    {
        ...
        LTSPacker::CancelRespond(process_result, pCancelOrder->serial_no, 0L, *cancle_order);
...
void LTSTunnel::QueryPosition(const T_QryPosition *pQryParam)
{
    ...
    ///经纪公司编号
    strncpy(qry_param.BrokerID, lib_cfg_->Logon_config().brokerid.c_str(), sizeof(TSecurityFtdcBrokerIDType));
    ///投资者编号
...
void LTSTunnel::QueryOrderDetail(const T_QryOrderDetail *pQryParam)
{
    ...
    strncpy(qry_param.BrokerID, lib_cfg_->Logon_config().brokerid.c_str(), sizeof(TSecurityFtdcBrokerIDType));
    ///交易所代码
    //strncpy(qry_param.ExchangeID, "", sizeof(TSecurityFtdcExchangeIDType));
    ///投资者编号
    strncpy(qry_param.InvestorID, lib_cfg_->Logon_config().investorid.c_str(), sizeof(TSecurityFtdcInvestorIDType));
...
void LTSTunnel::QueryTradeDetail(const T_QryTradeDetail *pQryParam)
{
    ...
    CSecurityFtdcQryTradeField qry_param;
    memset(&qry_param, 0, sizeof(CSecurityFtdcQryTradeField));
    ///经纪公司编号
    strncpy(qry_param.BrokerID, lib_cfg_->Logon_config().brokerid.c_str(), sizeof(TSecurityFtdcBrokerIDType));
    ///交易所代码
    //strncpy(qry_param.ExchangeID, "", sizeof(TSecurityFtdcExchangeIDType));
...
void LTSTunnel::QueryContractInfo(const T_QryContractInfo *pQryParam)
{
...
//empty, normal lts can't support quote now 20150313
// 询价
void LTSTunnel::ReqForQuoteInsert(const T_ReqForQuote *p)
{
    LogUtil::OnReqForQuote(p, tunnel_info_);
}
///报价录入请求
void LTSTunnel::ReqQuoteInsert(const T_InsertQuote *p)
{
    LogUtil::OnInsertQuote(p, tunnel_info_);
}
///报价操作请求
void LTSTunnel::ReqQuoteAction(const T_CancelQuote *p)
{
    LogUtil::OnCancelQuote(p, tunnel_info_);
}

void LTSTunnel::SetCallbackHandler(std::function<void(const T_OrderRespond *)> callback_handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_CancelRespond *)> callback_handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_OrderReturn *)> callback_handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_TradeReturn *)> callback_handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_PositionReturn *)> handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_OrderDetailReturn *)> handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_TradeDetailReturn *)> handler)
{
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_ContractInfoReturn *)> handler)
{
    TNL_LOG_DEBUG("LTSTunnel::SetCallbackHandler - QryContractInfoHandler_");
    LTSTradeInf * p_tunnel = static_cast<LTSTradeInf *>(trade_inf_);
    if (!p_tunnel)
    {
        TNL_LOG_ERROR("tunnel not init when SetCallbackHandler");
...
void LTSTunnel::SetCallbackHandler(std::function<void(const T_RtnForQuote *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_RtnForQuote)");
}
void LTSTunnel::SetCallbackHandler(std::function<void(const T_RspOfReqForQuote *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_RspOfReqForQuote)");
}
void LTSTunnel::SetCallbackHandler(std::function<void(const T_InsertQuoteRespond *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_InsertQuoteRespond)");
}
void LTSTunnel::SetCallbackHandler(std::function<void(const T_CancelQuoteRespond *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_CancelQuoteRespond)");
}
void LTSTunnel::SetCallbackHandler(std::function<void(const T_QuoteReturn *)> handler)
{
    TNL_LOG_WARN("tunnel not support function SetCallbackHandler(T_QuoteReturn)");
}
void LTSTunnel::SetCallbackHandler(std::function<void(const T_QuoteTrade *)> handler)
{
...
{
    return new LTSTunnel(tunnel_config_file);
}
