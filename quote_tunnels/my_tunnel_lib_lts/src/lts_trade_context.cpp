...
LTSTradeContext::LTSTradeContext()
{
    cur_req_id_ = 0;
...
void LTSTradeContext::InitOrderRef(OrderRefDataType orderref_prefix)
{
    cur_max_order_ref_ = orderref_prefix * 10000000;
}

// 设置报单引用（如果和前置建立多个会话时，取最大的，简化程序中报单引用的单调递增管理）
void LTSTradeContext::SetOrderRef(OrderRefDataType cur_max_order_ref)
{
...
OrderRefDataType LTSTradeContext::GetNewOrderRef()
{
    lock_guard<mutex> lock(sync_mutex_);
    ++cur_max_order_ref_;
...
int LTSTradeContext::GetRequestID()
{
    lock_guard<mutex> lock(sync_mutex_);
    return ++cur_req_id_;
}

void LTSTradeContext::SaveSerialNoToOrderRef(OrderRefDataType order_ref, const OriginalReqInfo &order_info)
{
...
const OriginalReqInfo * LTSTradeContext::GetOrderInfoByOrderRef(OrderRefDataType order_ref)
{
    lock_guard<mutex> lock(ref_mutex_);
...
void LTSTradeContext::UpdateCancelOrderRef(OrderRefDataType order_ref, const std::string &c_s_no)
{
    lock_guard<mutex> lock(ref_mutex_);
...
OrderRefDataType LTSTradeContext::GetOrderRefBySerialNo(long serial_no)
{
...
void LTSTradeContext::SetAnsweredPlaceOrder(OrderRefDataType order_ref)
{
...
void LTSTradeContext::SetAnsweredCancelOrder(OrderRefDataType order_ref)
{
    {
...
bool LTSTradeContext::IsAnsweredPlaceOrder(OrderRefDataType order_ref)
{
    lock_guard<mutex> lock(ans_po_mutex_);
    return answered_place_order_refs_.find(order_ref) != answered_place_order_refs_.end();
}

bool LTSTradeContext::IsAnsweredCancelOrder(OrderRefDataType order_ref)
{
    lock_guard<mutex> lock(ans_co_mutex_);
    return answered_cancel_order_refs_.find(order_ref) != answered_cancel_order_refs_.end();
}

bool LTSTradeContext::SaveOrderSysIDOfSerialNo(long sn, long id)
{
...
long LTSTradeContext::GetOrderSysIDOfSerialNo(long sn)
{
    lock_guard<mutex> lock(ordersysid_mutex_);
    SerialNoToOrderSysIDMap::iterator it = serialno_to_ordersysid_.find(sn);
    if (it == serialno_to_ordersysid_.end())
...
bool LTSTradeContext::CheckAndSetHaveHandleInsertErr(long sn)
{
...
bool LTSTradeContext::CheckAndSetHaveHandleActionErr(long sn)
{
