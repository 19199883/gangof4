...
MYQuoteData::MYQuoteData(const SubscribeContracts *subscribe_contracts, const std::string &provider_config_file)
{
    ...
    if (provider_config_file.empty()) { MY_LOG_ERROR("no quote provider config file"); }
...
bool MYQuoteData::InitInterface(const SubscribeContracts *subscribe_contracts, const ConfigData &cfg)
{
    ...
    interface_ = new DceUdpMD(subscribe_contracts, cfg);

    ConfigData cfgXspeed;
    cfgXspeed.Load("my_quote_dce_xspeed.config");
    interface1_ = new MyMdSpi(subscribe_contracts, cfgXspeed);
...
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDBestAndDeep_MY quote handler function not match quote provider."); }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const MDTenEntrust_MY*)> quote_handler)
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDTenEntrust_MY quote handler function not match quote provider."); }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const MDArbi_MY*)> quote_handler)
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDArbi_MY quote handler function not match quote provider."); }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const MDOrderStatistic_MY*)> quote_handler)
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDOrderStatistic_MY quote handler function not match quote provider."); }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const MDRealTimePrice_MY*)> quote_handler)
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDRealTimePrice_MY quote handler function not match quote provider."); }
}

void MYQuoteData::SetQuoteDataHandler(boost::function<void(const MDMarchPriceQty_MY*)> quote_handler)
{
    if (interface_) { ((DceUdpMD *) interface_)->SetQuoteDataHandler(quote_handler); }
    else { MY_LOG_ERROR("MDMarchPriceQty_MY quote handler function not match quote provider."); }
}

MYQuoteData::~MYQuoteData()
{
    if (interface_) delete ((DceUdpMD *) interface_);
    if (interface1_) delete ((MyMdSpi *) interface_);
}
