#ifndef _MY_TUNNEL_LIB_XELE_H_
#define _MY_TUNNEL_LIB_XELE_H_
...
class XeleTunnel : public MYTunnelInterface
{
public:
    /**
     * @param provider_config_file 配置文件名
     */
    XeleTunnel(const std::string &xele_config_file);
    virtual ~XeleTunnel();
...
private:
    // 禁止拷贝和赋值
    XeleTunnel(const XeleTunnel & other);
    XeleTunnel operator=(const XeleTunnel & other);
